...
import com.digitalasset.canton.crypto.{CryptoPureApi, SecureRandomness, Signature}
...
import com.digitalasset.canton.participant.protocol.submission.EncryptedViewMessageFactory.createRandomnessMap
            ...
            trackingLedgerEvents(participants.all, Seq.empty)(
              maliciousP1
                .submitCommand(
                  cmd,
                  confirmationRequestInterceptor =
                    // Retain the parent view, which is only sent to participant1 (1 recipient)
                    // rather than the subview which is sent to both (2 recipients).
                    _.focus(_.viewEnvelopes).modify { viewEnvelopes =>
                      Seq(viewEnvelopes.minBy(_.recipients.allRecipients.size))
                    },
                )
                .futureValueUS
            ),
                        ...
                        entry.shouldBeCantonError(
                          ...
                          msg =>
                            msg should include regex
                              raw"View \S+ lists a subview ViewHash(\S+), but we haven't received any views with this view hash",
                        ),
                      "p1 invalid subview reference alarm",
                    )
                  )
                } else {
                  // In the new decryption algorithm (PV`transparency`+), the parent view is considered invalid
                  // already during decryption if the subview is missing or fails decryption using the randomness
            ...
            LogEntry.assertLogSeq(
              Seq(
                (
                  { logEntry =>
                    if (testedProtocolVersion < ProtocolVersion.transparency) {
                      // Tags the duplicate envelope as a MalformedRejects.Payloads error
                      logEntry.shouldBeCantonError(
                        LocalRejectError.MalformedRejects.Payloads,
                        msg => msg should include("DuplicateLightViewTree"),
                      )
                    } else {
                      // Log a warning alarm when we detect a duplicate envelope. The duplicate envelope is filtered out.
                      logEntry.shouldBeCantonError(
                        SyncServiceAlarm,
                        _ should include regex
                          raw"Discarding duplicate envelope with ciphertext ID \S+ for participant \S+.",
                      )
                    }
                  },
    ...
    "a view has multiple different encryption keys" must_ { threat =>
      "alarm, discard, and reject the view" taggedAs_ (mitigation =>
        ...
        def replaceRandomnessForLightTransactionViewTree(
          ...
          // Fetch the child view, which is sent to both participants (2 recipients)
          // rather than the parent view which is only sent to participant1 (1 recipient).
          val envelope = tcr.viewEnvelopes.maxBy(_.recipients.allRecipients.size)
          ...
          // re-encrypt the view tree with a different randomness
          val newRandomness = pureCrypto.generateSecureRandomness(
            EncryptedViewMessage.computeRandomnessLength(pureCrypto)
          )
          val newSessionKey =
            pureCrypto.createSymmetricKey(newRandomness).valueOrFail("create new session key")
          ...
          val newSessionKeyMap = createRandomnessMap(
            LazyList(participant1.id, participant2.id),
            newRandomness,
            crypto,
            deterministicEncryption = false,
          ).map(_.values.toSeq)
            .futureValueUS
            .valueOrFail("create new randomness map")

          val newMessage = EncryptedViewMessageFactory
            .encryptView(TransactionViewType)(
              viewTree,
              (newSessionKey, newSessionKeyMap),
          ...
          tcr.focus(_.viewEnvelopes).modify(_ :+ newEnvelope)
        }
