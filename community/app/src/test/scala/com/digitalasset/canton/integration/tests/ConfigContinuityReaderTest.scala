  ...
  private lazy val allTransforms: Map[(Int, Int, Int), Transforms] = Map(
    ...
    (3, 6, 0) -> Transforms(
      Seq(
        ...
        // use-new-client and use-new-processor were removed in 3.6 after being deprecated in 3.5
        Seq("use-new-client", "use-new-processor").flatMap { key =>
          Seq(
            s"canton.participants.participant1.topology.$key",
            s"canton.sequencers.sequencer1.topology.$key",
            s"canton.mediators.mediator1.topology.$key",
          )
        }
    ),
  )
