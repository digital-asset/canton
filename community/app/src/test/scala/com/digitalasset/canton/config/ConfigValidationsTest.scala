...
class ConfigValidationsTest extends BaseTestWordSpec {
  ...
  "nodes with crypto scheme configurations" should {
    ...
    "fail config validation when default crypto scheme is unsupported" in {
      ...
      assertErrors(configInvalid)(
        s"participant p1: The selected signing algorithm specification, ${SigningAlgorithmSpec.Ed25519}, " +
          s"is not supported. Supported algorithms: ${NonEmpty.mk(Set, SigningAlgorithmSpec.EcDsaSha256)}.",
        s"participant p1: The selected signing key specification, ${SigningKeySpec.EcCurve25519}, " +
          s"is not supported. Supported keys: ${NonEmpty.mk(Set, SigningKeySpec.EcP256)}.",
        s"participant p1: The selected encryption algorithm specification, ${EncryptionAlgorithmSpec.EciesHkdfHmacSha256Aes128Cbc}, " +
          s"is not supported. Supported algorithms: ${NonEmpty.mk(Set, EncryptionAlgorithmSpec.RsaOaepSha256)}.",
        s"participant p1: The selected encryption key specification, ${EncryptionKeySpec.EcP256}, " +
    ...
    "fail config validation when key specs aren’t covered by the supported algorithms" in {
      ...
      assertErrors(signingKeySpecsInvalid)(
        s"participant p1: The allowed signing key specifications (${NonEmpty.mk(Set, SigningKeySpec.EcP256)}) are not all supported by the allowed signing " +
          s"algorithms. Supported keys for those algorithms are: ${SigningAlgorithmSpec.EcDsaSha384.supportedSigningKeySpecs}.",
        s"participant p1: The signing algorithm specification ${SigningAlgorithmSpec.EcDsaSha384} does not include any key " +
      ...
      assertErrors(encryptionKeySpecsInvalid)(
        s"participant p1: The allowed encryption key specifications (${NonEmpty
            .mk(Set, EncryptionKeySpec.EcP256)}) are not all supported by the allowed encryption " +
          s"algorithms. Supported keys for those algorithms are: ${EncryptionAlgorithmSpec.RsaOaepSha256.supportedEncryptionKeySpecs}.",
        s"participant p1: The encryption algorithm specification ${EncryptionAlgorithmSpec.RsaOaepSha256} does not include any key " +
  ...
  "nodes using session signing keys" should {
    ...
    "fail config validation when session signing keys are enabled without a KMS unless non-standard config is set" in {
      ...
      assertErrors(config)(
        "participant p1: Session signing keys should only be enabled when using a KMS provider, as they generate extra traffic " +
    ...
    "fail config validation with unsupported session signing algorithm specification" in {
      ...
      assertErrors(config)(
        s"participant p1: The selected signing algorithm specification, $signingAlgorithmSpec, for session signing keys is not " +
    ...
    "fail config validation with unsupported session signing key specification" in {
      ...
      assertErrors(config)(
        s"participant p1: The selected signing key specification, $sessionSigningKeySpec, for session signing keys is not " +
          s"supported. Supported keys: $supportedSigningKeySchemes."
      )
    }

    "fail config validation if parameters are out of bounds" in {
      val participantNodeConfig = ParticipantNodeConfig(
        crypto = CryptoConfig(
          provider = Kms,
          kms = Some(KmsConfig.Aws.defaultTestConfig),
      ...
      def changeSessionSigningKeyParams(
          ...
          toleranceShiftDuration: Option[NonNegativeFiniteDuration] = None,
        ...
        val params = SessionSigningKeysConfig.default.copy(
          ...
          keyEvictionPeriod = keyEvictionPeriod.getOrElse(defaultKeyEvictionPeriod),
        )
          ...
          .replace(
            Map(
              InstanceName.tryCreate("p1") ->
                participantNodeConfig.focus(_.crypto.sessionSigningKeys).replace(params)
            )
          )
      ...
      val minValidityDuration =
        PositiveFiniteDuration.tryFromDuration(
          ConfigValidations.minValidityDuration(
            defaultToleranceShiftDuration.duration,
            defaultCutOffDuration.duration,
          )
        )

      // we expect the validity duration to be strictly greater than our computed minimum.
      val invalidKeyValidityDuration = minValidityDuration
      assertErrors(
        changeSessionSigningKeyParams(
          keyValidityDuration = Some(invalidKeyValidityDuration)
        )
      )(
        s"participant p1: The selected session signing key validity duration of $invalidKeyValidityDuration " +
          s"is too short. It must be longer " +
          s"than $minValidityDuration."
      )

      val invalidToleranceShiftDuration = defaultCutOffDuration
      assertErrors(
        changeSessionSigningKeyParams(
          keyValidityDuration = None,
          toleranceShiftDuration = Some(invalidToleranceShiftDuration),
        )
      )(
      ...
      assertErrors(
        changeSessionSigningKeyParams(
          keyValidityDuration = None,
          keyEvictionPeriod = Some(invalidKeyEvictionPeriod),
        )
      )(
    ...
    "pass config validation with parameters out of bounds and checks disabled" in {
      ...
      assertErrors(config.copy(parameters = config.parameters.copy(nonStandardConfig = false)))(
        s"participant p1: Disabling bound checks on session signing key validity parameters is only allowed with a " +
