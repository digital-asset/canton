...
abstract class LsuMediatorInspectionServiceIntegrationTest
  ...
  override protected lazy val upgradeTime: CantonTimestamp = CantonTimestamp.Epoch.plusSeconds(15)
  ...
  // Allow to complete promises for the held-back confirmation responses
  // so they don't block the closing of the sequencer.
  protected val completeAtEnd: ArrayBuffer[Promise[Unit]] = ArrayBuffer.empty

  override def afterAll(): Unit = {
    completeAtEnd.foreach(_.trySuccess(()))
    super.afterAll()
  }
}
...
final class LsuMediatorVerdictsUnconfirmedAndConfirmedIntegrationTest
  ...
  "LSU upgrade" should {
    "cause the verdicts stream on the old mediator to be flushed and completed" in { implicit env =>
      ...
      val (unconfirmedSubmissionF, confirmedSubmissionF) =
        withClue("create an unconfirmed request, followed by a confirmed one") {
          ...
          completeAtEnd.append(takeNextResponsePromise()) // So it doesn't interfere with shutdown
          ...
          (unconfirmedSubmissionF, confirmedSubmissionF)
        }
      ...
      val confirmedRecordTime = withClue("move past LSU upgrade time") {
        loggerFactory.assertEventuallyLogsSeq(SuppressionRule.LevelAndAbove(Level.WARN))(
          ...
          logs => {
            ...
            forExactly(1, logs)(
              _.warningMessage should include("expired, but readers are still active")
            )
          },
        )
        transferTraffic()
