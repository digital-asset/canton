      ...
      loggerFactory.assertLogsUnorderedOptional(
        {
          ...
          def getStartupConnectionRetryDelay(p: LocalParticipantReference): Long =
            p.underlying.value.config.sequencerClient.startupConnectionRetryDelay.underlying.toNanos

          val retryDelay = Duration.ofNanos(
            getStartupConnectionRetryDelay(participant2)
              .max(getStartupConnectionRetryDelay(participant3))
          )
