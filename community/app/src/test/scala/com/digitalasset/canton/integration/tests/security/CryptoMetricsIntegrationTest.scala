...
trait CryptoMetricsIntegrationTest {
  ...
  "signing, decryption latencies, and KMS metrics are recorded" in { implicit env =>
    ...
    // confirmation of participant1, with participant 2 as the observer.
    // It involves the following signing and decryption operations:
    ...
    // Total decryption operations: participant1 = 1 + 1 = 2, participant2 = 1 + 1 = 2
    val expectedDeltas = Map(
      participant1 -> MetricCounts(signingCount = 7, decryptCount = 2),
      participant2 -> MetricCounts(signingCount = 5, decryptCount = 2),
    )
