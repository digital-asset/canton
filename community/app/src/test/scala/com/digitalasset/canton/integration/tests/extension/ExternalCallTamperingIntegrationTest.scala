...
/** End-to-end coverage for a malicious participant that tampers with the recorded external-call
  ...
  * version 36 onwards, so the tests are gated on v36. The test package targets LF 2.4, which lies
  * outside the participant's default (stable) LF range, so dev version support is enabled on the
  * participants.
  */
