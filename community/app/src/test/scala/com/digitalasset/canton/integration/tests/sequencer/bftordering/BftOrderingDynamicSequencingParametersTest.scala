    ...
    "be able to change sequencing parameters" onlyRunWithOrGreaterThan ProtocolVersion.v35 in {
      implicit env =>
        ...
        sequencer1.bft
          ...
          .pbftViewChangeTimeout shouldBe SequencingParameters.DefaultPbftViewChangeTimeout

        val newPbftViewChangeTimeout = PositiveFiniteDuration(5.seconds).toInternal
        require(newPbftViewChangeTimeout != SequencingParameters.DefaultPbftViewChangeTimeout)
        val newSegmentLength = SegmentLength(PositiveLong.tryCreate(100))
        require(newSegmentLength != SequencingParameters.DefaultSegmentLength)
        val newSequencingParameters =
          SequencingParameters.create(
            newPbftViewChangeTimeout,
            SegmentLength(PositiveLong.tryCreate(100)),
            DefaultLeaderSelectionPolicyConfig,
          )(
