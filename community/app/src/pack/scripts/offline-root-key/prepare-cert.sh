...
usage() {
  ...
  echo "  --root-key-spec <value>        Optional. Key specification for the root public key. Valid values are:"
...
EOF
    ...
    --root-key-spec)
      ROOT_KEY_SPEC="$2"
      case "$ROOT_KEY_SPEC" in
        ed25519)
          ROOT_KEY_SPEC="SIGNING_KEY_SPEC_EC_CURVE25519"
          ;;
        ecp256)
          ROOT_KEY_SPEC="SIGNING_KEY_SPEC_EC_P256"
          ;;
        ecp384)
          ROOT_KEY_SPEC="SIGNING_KEY_SPEC_EC_P384"
          ;;
        secp256k1)
...
# For a new delegation, build it from the key
  ...
  # If the root key spec is not set, try to detect it
  if [[ -z "$ROOT_KEY_SPEC" ]]; then
