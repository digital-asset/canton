...
import com.digitalasset.canton.ledger.sandbox.bridge.validate.SequencerState.SequencerQueue
...
class SequencerStateSpec extends AnyFlatSpec with Matchers {
  ...
  "enqueue" should "update the sequencer state" in {
    inside((1L to 3L).map(toOffset)) { case Seq(offset1, offset2, offset3) =>
  ...
  "dequeue" should "evict entries" in {
    inside((0L to 5L).map(toOffset)) {
      case Seq(offsetBefore, offset1, offset2, offset3, offset4, offsetAfter) =>
        ...
        populatedSequencerState
          ...
          .dequeue(offset4)
          .tap {
            _ shouldBe SequencerState.empty
          }
          .dequeue(offsetAfter)
          .tap {
            _ shouldBe SequencerState.empty
