  ...
  private def readThroughKeyCache(
    ...
    if (pageToken.isEmpty) {
        ...
        .putAsync(
          ...
          validAtEventSeqId =>
            contractsReader
              ...
              .map { case (contractIdsWithSeqIds, nextPageToken) =>
                fullResultPromise.success((contractIdsWithSeqIds.map(_._1), nextPageToken))
                contractIdsWithSeqIds.headOption.fold[ContractKeyStateValue](Empty) {
                  case (cid, eventSeqId) =>
                    Last(
                      contractId = cid,
      ...
      contractsReader
        ...
        .map { case (contractIdsWithSeqIds, nextPageToken) =>
          (contractIdsWithSeqIds.map(_._1), nextPageToken)
        }
    }
}
