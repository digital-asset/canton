...
import com.daml.ledger.api.v2.command_submission_service.SubmitReassignmentRequest
...
class SubmitRequestValidator(
  ...
  def validateReassignment(
    ...
    for {
      ...
      workflowId <- validateOptional(Some(reassignmentCommand.workflowId).filter(_.isEmpty))(
