...
import com.digitalasset.canton.ledger.api.tls.{SecretsUrl, TlsConfiguration}
import com.digitalasset.canton.platform.apiserver.SeedService.Seeding
import com.digitalasset.canton.platform.apiserver.{AuthServiceConfig, AuthServiceConfigCli}
  ...
  private def parser[Extra](
    ...
    OParser.sequence(
      ...
      opt[Int]("max-commands-in-flight")
        ...
        .text(
          s"Maximum number of submitted commands for which the CommandService is waiting to be completed in parallel. Default is ${CommandConfiguration.DefaultMaxCommandsInFlight}."
        ),
      opt[Int]("input-buffer-size")
        .optional()
        .action((_, config) => config)
        .text(
          s"Deprecated parameter -- tracking commands per party is not supported anymore."
        ),
      opt[Duration]("tracker-retention-period")
        .optional()
        .action((_, config) => config)
        .text(
          "Deprecated parameter -- tracking commands per party is not supported anymore."
        ),
