...
import com.digitalasset.canton.util.retry.{Backoff, Success as RetrySuccess}
...
class PersistentUserManagementStore(
  ...
  private def inTransaction[T](
    ...
    implicit val success = RetrySuccess.always
    val retry = Backoff(
      logger = logger,
      flagCloseable = flagCloseable,
      maxRetries = 10,
...
object RetryOnceMoreExceptionRetryPolicy extends ExceptionRetryable {
  override def retryOK(outcome: Try[_], logger: TracedLogger, lastErrorKind: Option[ErrorKind])(
    ...
    case Failure(RetryOnceMoreException(_)) => SpuriousTransientErrorKind
    case Failure(_) => FatalErrorKind
  }
}
