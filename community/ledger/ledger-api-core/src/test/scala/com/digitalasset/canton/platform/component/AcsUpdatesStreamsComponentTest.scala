...
trait AcsUpdatesStreamsComponentTest extends AnyWordSpec with IndexComponentTest {
  ...
  "acsUpdates" should {
    ...
    "emit the right ACS updates for a mix of a create, an archive, two ACS commitments and a topology transaction" in {
      ...
      ingestTopologyEvents(
        parties = Set("mixed-topology-party"),
        synchronizerParametersPayloads =
          Seq("synchronizer-parameters-0", "synchronizer-parameters-1"),
      ...
      acsUpdates shouldBe Seq[AcsUpdate](
        ...
        AcsUpdate.EffectiveTopologyUpdate(
          ...
          Some(
            Update.TopologyTransactionEffective.GenericTopologyEvent.SynchronizerParametersState(
              ByteString.copyFromUtf8("synchronizer-parameters-1")
            )
          ),
        ),
      )
    ...
    "emit an effective topology update carrying only synchronizer parameters" in {
      val rangeStart = index.currentLedgerEnd().map(_.lastOffset)

      val parameterPayloads =
        Seq("synchronizer-parameters-1", "synchronizer-parameters-2", "synchronizer-parameters-3")
      ingestTopologyEvents(
        synchronizerParametersPayloads = parameterPayloads,
        synchronizerId = synchronizer1,
        recordTime = nextRecordTime(),
      ).discard
      ...
      acsUpdates.loneElement shouldBe
        AcsUpdate.EffectiveTopologyUpdate(
          partyToParticipantMappings = Set.empty,
          synchronizerParameterState = Some(
            Update.TopologyTransactionEffective.GenericTopologyEvent.SynchronizerParametersState(
              ByteString.copyFromUtf8("synchronizer-parameters-3")
            )
          ),
        )
    }
    ...
    "emit an effective topology update carrying both party authorizations and synchronizer parameters" in {
      ...
      ingestTopologyEvents(
        parties = Set("combined-topology-party"),
        synchronizerParametersPayloads =
          Seq("synchronizer-parameters-11", "synchronizer-parameters-12"),
        synchronizerId = synchronizer1,
        recordTime = nextRecordTime(),
      ).discard
      ...
      acsUpdates.loneElement shouldBe
        AcsUpdate.EffectiveTopologyUpdate(
          partyToParticipantMappings = Set(
            Update.TopologyTransactionEffective.TopologyEvent.PartyToParticipantAuthorization(
              party = Ref.Party.assertFromString("combined-topology-party"),
              participant = Ref.ParticipantId.assertFromString("participant"),
              authorizationEvent = Update.TopologyTransactionEffective.AuthorizationEvent
                .Onboarding(Update.TopologyTransactionEffective.AuthorizationLevel.Observation),
            )
          ...
          synchronizerParameterState = Some(
            Update.TopologyTransactionEffective.GenericTopologyEvent.SynchronizerParametersState(
              ByteString.copyFromUtf8("synchronizer-parameters-12")
            )
          ),
        )
    }
    ...
    "ignore updates from a different synchronizer" in {
      ...
      val topo1 = topologyTx(synchronizer1, "synchronizer1-parameters")
      val topo2 = topologyTx(synchronizer2, "synchronizer2-parameters")
  ...
  "acs" should {
    "return active contracts that have at least one stakeholder in stakeholders1" in {
      ...
      contractsParty1All.map(_.copy(continuationToken = ByteString.empty())) shouldBe Seq(
      ...
      contractsParty2All.map(_.copy(continuationToken = ByteString.empty())) shouldBe Seq(
        InternalIndexService.ActiveContract(
