...
import com.digitalasset.canton.ledger.runner.common.ConfigLoaderSpec.{
  ComplexObject,
  InnerObject,
  TestScope,
}
...
class ConfigLoaderSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  behavior of "ConfigLoader.loadConfig for com.digitalasset.canton.ledger.runner.common.Config"
...
object ConfigLoaderSpec {
  final case class InnerObject(value1: String, value2: String)
  object InnerObject {
  ...
  trait TestScope {
    ...
    val config = TestResourceUtils.resourceFile("config/testp.conf")

    val testFileConfig = TestResourceUtils.resourceFile("config/test.conf")

    val testFileConfig2 = TestResourceUtils.resourceFile("config/test2.conf")
  }
}
