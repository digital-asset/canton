...
import com.digitalasset.canton.ledger.runner.common.CliConfigSpec.TestScope
...
final class CliConfigSpec
  ...
  it should "get the jdbc string from the environment when provided" in new TestScope {
    val config = configParser(
      ...
      PartialFunction.condOpt(_) { case `jdbcEnvVar` => TestJdbcValues.jdbcFromEnv },
...
object CliConfigSpec {
  ...
  trait TestScope extends Matchers {
    ...
    def confStringPath = "config/test.conf"
    def confFile = TestResourceUtils.resourceFile(confStringPath)
    def confFilePath = confFile.getAbsolutePath

    def confStringPath2 = "config/test2.conf"
    def confFile2 = TestResourceUtils.resourceFile(confStringPath2)
    def confFilePath2 = confFile2.getAbsolutePath
  }
}
