    ...
    val indexResourceOwner =
      for {
        ...
        indexService <- new IndexServiceOwner(
          dbSupport = dbSupport,
          initialLedgerId = LedgerId(IndexComponentTest.TestLedgerId),
      ...
      Source(
        List(
          LedgerInitialConditions(
            ...
            config =
              Configuration(0L, LedgerTimeModel.reasonableDefault, Duration.ZERO), // not used
            initialRecordTime = Time.Timestamp(0L), // not used
          )
        )
      )
