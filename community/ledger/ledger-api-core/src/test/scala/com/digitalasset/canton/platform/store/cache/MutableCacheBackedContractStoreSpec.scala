...
class MutableCacheBackedContractStoreSpec
  ...
  "lookupNonUniqueContractKey" should {
    "cap the limit and log when the requested limit exceeds the configured max" in {
      ...
      when(
        ...
        Future.successful(
          (Vector.empty[(ContractId, Long)], Option.empty[Long])
        )
      )
  ...
  "readThroughKeyCache" should {
    "read-through and populate the cache via putAsync when pageToken is None" in {
      ...
      val spyReader =
        spy(ConfigurableContractsReaderFixture((Vector(cId_1 -> eventSeqId1), None)))
      for {
        store <- contractStore(
          cachesSize = 10L,
          loggerFactory = loggerFactory,
    ...
    "query the DB directly without populating the cache when pageToken is Some(...)" in {
      ...
      val spyReader =
        spy(ConfigurableContractsReaderFixture((Vector(cId_2 -> eventSeqId3), None)))
      for {
        store <- contractStore(
          cachesSize = 10L,
          loggerFactory = loggerFactory,
      ...
      val spyReader =
        spy(
          ConfigurableContractsReaderFixture(
            (Vector(cId_1 -> eventSeqId1, cId_2 -> eventSeqId2), None)
          )
        )
      for {
        store <- contractStore(
    ...
    "cache Last with thereMightBeMore = true when the DB returns a single contract but a continuation token" in {
      ...
      val spyReader =
        spy(
          ConfigurableContractsReaderFixture((Vector(cId_1 -> eventSeqId1), Some(eventSeqId1)))
        )
      for {
        store <- contractStore(
          cachesSize = 10L,
    ...
    "cache Empty when the DB returns no contracts" in {
      ...
      val spyReader =
        spy(ConfigurableContractsReaderFixture((Vector.empty, None)))
      for {
        store <- contractStore(
          cachesSize = 10L,
          loggerFactory = loggerFactory,
    ...
    override def lookupNonUniqueKey(
      ...
      (key, notEarlierThanEventSeqId) match {
        case (`someKey`, `eventSeqId0`) =>
          Future.successful((Vector(cId_1 -> eventSeqId0), None))
        case (`someKey`, `eventSeqId2`) =>
          Future.successful((Vector(cId_2 -> eventSeqId2), None))
        case _ => Future.successful((Vector.empty, None))
      }
  ...
  case class ContractsReaderFixtureWithNuck(nuckKey: Key) extends LedgerDaoContractsReader {
    ...
    override def lookupNonUniqueKey(
      ...
      if (key == nuckKey)
        ...
        Future.successful((Vector.empty, None))
  }

  @SuppressWarnings(Array("org.wartremover.warts.FinalCaseClass")) // This class is spied in tests
  case class ConfigurableContractsReaderFixture(
      dbResult: (Vector[(ContractId, Long)], Option[Long])
