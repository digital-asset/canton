...
class InMemoryStateUpdaterSpec
  ...
  "updateCaches" should "not update cachesUpdatedUpto if contractStateCaches.push throws" in new Scope {
    ...
    InMemoryStateUpdater.updateCaches(
      ...
      ledgerEnd41,
      traceContext,
      contractStateEvents = extractContractStateEvents(updates),
    )
...
object InMemoryStateUpdaterSpec {
  ...
  trait Scope
    ...
    val prepareResultOnlyReassignment: PrepareResult = PrepareResult(
      ...
      emptyTraceContext,
      traceContext,
      contractStateEvents = extractContractStateEvents(Vector(assignLogUpdate)),
    )
    ...
    val prepareResultWithEmptyFlatEventWitnesses: PrepareResult = PrepareResult(
      ...
      emptyTraceContext,
      traceContext,
      contractStateEvents = extractContractStateEvents(emptyFlatWitnessUpdates),
    )
  ...
  // TODO(#31713) Spell out the expected ContractStateEvents explicitly in each test.
  private def extractContractStateEvents(
    ...
    updates.flatMap {
      case tx: TransactionLogUpdate.TransactionAccepted =>
        tx.events.collect {
          case created: TransactionLogUpdate.CreatedEvent if created.flatEventWitnesses.nonEmpty =>
            val event = ContractStateEvent.Activated(
              contractId = created.contractId,
              globalKey = created.contractKey.zip(created.createKeyHash).map { case (k, kh) =>
                Key.assertBuild(created.templateId, created.packageName, k.unversioned, kh)
              },
              eventSequentialId = seqId,
              isInitial = true,
            )
            seqId += 1
            event
          case exercised: TransactionLogUpdate.ExercisedEvent
              if exercised.consuming && exercised.flatEventWitnesses.nonEmpty =>
            ContractStateEvent.Deactivated(
              contractId = exercised.contractId,
              globalKey = exercised.contractKey.flatMap(k =>
                exercised.contractKeyHash.map(hash =>
                  Key.assertBuild(exercised.templateId, exercised.packageName, k.unversioned, hash)
                )
              ),
              deactivatedEventSequentialId = 0L,
              isFinal = true,
            )
        }
      case re: TransactionLogUpdate.ReassignmentAccepted =>
        re.reassignment.map {
          case assign: Reassignment.Assign =>
            val event = ContractStateEvent.Activated(
              contractId = assign.createNode.coid,
              globalKey = assign.createNode.keyOpt.map(_.globalKey),
              eventSequentialId = seqId,
              isInitial = false,
            )
            seqId += 1
          ...
          case unassign: Reassignment.Unassign =>
            ContractStateEvent.Deactivated(
              ...
              globalKey = unassign.keyOpt.map(_.globalKey),
              deactivatedEventSequentialId = 0L,
              isFinal = false,
            )
        }
      ...
      override def default: NonEmptyVector[ContractStateEvent] = {
        ...
        NonEmptyVector.one(
          ContractStateEvent.Activated(
            ...
            globalKey =
              createdEvent.contractKey.zip(createdEvent.createKeyHash).map { case (k, kh) =>
                Key.assertBuild(
