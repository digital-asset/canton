...
create table ord_pbft_messages_completed(
  ...
  primary key (epoch_number, block_number, from_sequencer_id, discriminator)
) partition by range (epoch_number);
...
create index idx_ord_metadata_output_blocks_bft_ts on ord_metadata_output_blocks(bft_ts);
create index idx_ord_metadata_output_blocks_block_number on ord_metadata_output_blocks(block_number);
...
create function create_partitions_for_epoch_range(
...
declare
  ...
  max_epoch bigint;
begin
  -- Use a shared epoch range across ord_epochs_old and ord_availability_batch_old.
  -- ord_availability_batch_old can contain epochs ahead of ord_epochs_old,
  -- while other transient tables (ord_pbft_messages_in_progress) may be empty during the migration.
  select min(epoch_number), max(epoch_number)
  into min_epoch, max_epoch
  from (
    select epoch_number from ord_epochs_old
    union all
    select epoch_number from ord_availability_batch_old
  ) all_epochs;

  if min_epoch is null or max_epoch is null then
    min_epoch := 0;
    max_epoch := 0;
  end if;

  perform create_partitions_for_epoch_range('ord_epochs'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_availability_batch'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_pbft_messages_in_progress'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_pbft_messages_completed'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_metadata_output_blocks'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_metadata_output_epochs'::regclass, min_epoch, max_epoch);
  perform create_partitions_for_epoch_range('ord_leader_selection_state'::regclass, min_epoch, max_epoch);
end $$;
