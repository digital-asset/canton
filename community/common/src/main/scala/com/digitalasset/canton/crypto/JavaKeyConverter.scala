...
object JavaKeyConversionError {
  ...
  case class GeneralError(error: Exception) extends JavaKeyConversionError {
    override def pretty: Pretty[GeneralError] =
      prettyOfClass(unnamedParam(_.error))
  }

  case class UnsupportedAlgorithm(algorithmIdentifier: AlgorithmIdentifier)
  ...
  case class UnsupportedKeyFormat(format: CryptoKeyFormat, expectedFormat: CryptoKeyFormat)
      extends JavaKeyConversionError {
  ...
  case class KeyStoreError(error: String) extends JavaKeyConversionError {
    override def pretty: Pretty[KeyStoreError] =
      prettyOfClass(unnamedParam(_.error.unquoted))
  }

  case class InvalidKey(error: String) extends JavaKeyConversionError {
