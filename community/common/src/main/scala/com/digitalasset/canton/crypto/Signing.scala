...
case class Signature private[crypto] (
...
case class SigningPublicKeyWithName(
...
object SigningKeyGenerationError {
  ...
  case class GeneralError(error: Exception) extends SigningKeyGenerationError {
    override def pretty: Pretty[GeneralError] = prettyOfClass(unnamedParam(_.error))
  }

  case class NameInvalidError(error: String) extends SigningKeyGenerationError {
    override def pretty: Pretty[NameInvalidError] = prettyOfClass(unnamedParam(_.error.unquoted))
  }

  case class FingerprintError(error: String) extends SigningKeyGenerationError {
    override def pretty: Pretty[FingerprintError] = prettyOfClass(unnamedParam(_.error.unquoted))
  }

  case class UnsupportedKeyScheme(scheme: SigningKeyScheme) extends SigningKeyGenerationError {
    override def pretty: Pretty[UnsupportedKeyScheme] = prettyOfClass(param("scheme", _.scheme))
  }

  case class SigningPrivateStoreError(error: CryptoPrivateStoreError)
      extends SigningKeyGenerationError {
    override def pretty: Pretty[SigningPrivateStoreError] = prettyOfClass(unnamedParam(_.error))
  }

  case class SigningPublicStoreError(error: CryptoPublicStoreError)
      extends SigningKeyGenerationError {
...
object SignatureCheckError {
  ...
  case class InvalidSignature(signature: Signature, bytes: ByteString, error: String)
  ...
  case class InvalidCryptoScheme(message: String) extends SignatureCheckError {
    override def pretty: Pretty[InvalidCryptoScheme] = prettyOfClass(
      unnamedParam(_.message.unquoted)
    )
  }
  case class InvalidKeyError(message: String) extends SignatureCheckError {
    override def pretty: Pretty[InvalidKeyError] = prettyOfClass(unnamedParam(_.message.unquoted))
  }
  case class GeneralError(error: Exception) extends SignatureCheckError {
    override def pretty: Pretty[GeneralError] = prettyOfClass(unnamedParam(_.error))
  }
  case class SignatureWithWrongKey(message: String) extends SignatureCheckError {
    override def pretty: Pretty[SignatureWithWrongKey] = prettyOfClass(
      unnamedParam(_.message.unquoted)
    )
  }
  case class SignerHasNoValidKeys(message: String) extends SignatureCheckError {
