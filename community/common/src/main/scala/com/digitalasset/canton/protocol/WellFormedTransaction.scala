...
object WellFormedTransaction {
  ...
  def check[S <: Stage](
    ...
    val result = for {
      ...
      _ <- checkNoContractIdsInKeysAndNonemptyMaintainers(normalizedTx)
  ...
  private def normalizeNodeIds(
    ...
    // Leverage upstream code from  VersionedTransaction, once it has been created
    val normalization = tx.fold(Map.empty[NodeId, NodeId]) { case (remapping, (oldNodeId, _)) =>
