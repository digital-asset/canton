...
/** Configuration for where to record sequencer sends and events to.
  ...
  */
case class RecordingConfig(directory: Path, filename: Option[String] = None) {
...
/** Configuration for setting up a sequencer client to replay requests or received events.
  ...
  */
case class ReplayConfig(recordingConfig: RecordingConfig, action: ReplayAction)
...
object ReplayAction {
  ...
  /** Replay sends that were made to the sequencer.
    ...
    */
  case class SequencerSends(
      sendTimeout: NonNegativeFiniteDuration = NonNegativeFiniteDuration.ofSeconds(20),
