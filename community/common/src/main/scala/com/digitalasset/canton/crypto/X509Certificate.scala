    ...
    def signedCertificate(
      ...
      for {
        ...
        certificate <- getCertificate(
          new SignatureProvider(signature.toByteString(ProtocolVersion.default))
        )
...
object X509CertificateError {
  case class DecodingError(reason: String) extends X509CertificateError
  case class EncodingError(reason: String) extends X509CertificateError
  case class SigningError(reason: crypto.SigningError) extends X509CertificateError
  case class InvalidPublicKey(key: Fingerprint, reason: String) extends X509CertificateError
  case class KeyStoreError(key: Fingerprint, reason: CryptoPublicStoreError)
      extends X509CertificateError
  case class InvalidSubjectAlternativeNames(subjectAlternativeNames: Seq[String], reason: String)
      extends X509CertificateError
