...
trait JavaPrivateKeyConverterTest extends BaseTest with CryptoTestHelper { this: AsyncWordSpec =>
  ...
  private def javaConvertTest(
    ...
    s"Convert $name private keys to Java keys" should {
      ...
      "Convert private key to Java" in {
        for {
          ...
          _ = crypto.javaKeyConverter.toJava(privateKey).valueOrFail("convert to java")
        } yield assert(true)
      }
    }
  }

  def javaPrivateKeyConverterProvider(
    ...
    forAll(supportedSigningKeySchemes) { signingKeyScheme =>
      ...
      s"Convert with $signingKeyScheme private signing keys" should {
        ...
        "Convert signing keys of certificates to Java key store" in {
          for {
            ...
            publicKey1 <- getSigningPublicKey(crypto, signingKeyScheme)
            cryptoPrivateStore = crypto.cryptoPrivateStore.toExtended
              .valueOrFail("crypto private store does not implement all necessary methods")
            ...
            publicKey2 <- getSigningPublicKey(crypto, signingKeyScheme)
