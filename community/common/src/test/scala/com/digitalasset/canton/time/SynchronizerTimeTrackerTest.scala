...
class SynchronizerTimeTrackerTest extends FixtureAsyncWordSpec with BaseTest {
  ...
  class Env {
    ...
    val synchronizerTimeTrackerConfig = SynchronizerTimeTrackerConfig(
  ...
  "ensure minimum time interval" should {
    "should ask for time if a sufficient amount of local time progresses" in { env =>
      ...
      clock.advance(synchronizerTimeTrackerConfig.minObservationDuration.asJava.plusMillis(1))
      ...
      for {
        ...
        _ = clock.advance(
          synchronizerTimeTrackerConfig.minObservationDuration.asJava.minusSeconds(1)
        )
