...
object MerkleTreeTest {
  ...
  object AbstractLeaf extends HasProtocolVersionedCompanion[VersionedAbstractLeaf] {
    ...
    def fromProto(protobufVersion: Int)(bytes: ByteString): ParsingResult[Leaf1] = {
      leafFromByteString(i =>
        Leaf1(i)(protocolVersionRepresentativeFor(ProtobufVersion(protobufVersion)))
      )(
        bytes
      )
        .leftMap(e => ProtoDeserializationError.OtherError(e.message))
    }
  }
