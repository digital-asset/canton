...
class MetadataHashTest extends BaseTest with AnyWordSpecLike with Matchers with HashUtilsTest {
  ...
  "Metadata Encoding" should {
    ...
    "be stable" in {
      val actual = TransactionMetadataHasher(HashingSchemeVersion.V2)
        .tryHashMetadata(metadata(SerializationVersion.V1))
        .toHexString
    ...
    "explain encoding" in {
      ...
      val actualMetadataHash =
        TransactionMetadataHasher(HashingSchemeVersion.V2)
          .tryHashMetadata(metadata(SerializationVersion.V1), hashTracer)
