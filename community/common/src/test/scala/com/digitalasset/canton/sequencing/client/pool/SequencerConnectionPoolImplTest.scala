...
class SequencerConnectionPoolImplTest
    extends AnyWordSpec
    with BaseTest
  ...
  "SequencerConnectionPool" should {
    ...
    "signal when the threshold cannot be reached during validation" in {
      withConnectionPool(
        ...
        inside(pool.start().futureValueUS) {
          case Left(SequencerConnectionPoolError.ThresholdUnreachableError(message)) =>
    ...
    "signal when initialization times out" in {
      ...
      withConnectionPool(
        ...
        inside(pool.start().futureValueUS) {
          case Left(SequencerConnectionPoolError.TimeoutError(message)) =>
    ...
    "take into account an expected physical synchronizer ID" in {
      withConnectionPool(
        ...
        ) { case Left(SequencerConnectionPoolError.InvalidConfigurationError(error)) =>
  ...
  private def contentsShouldEqual(
      contents: Map[SequencerId, Set[SequencerConnection]],
      created: Map[SequencerId, Set[InternalSequencerConnection]],
  ): Assertion = {
