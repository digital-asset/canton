...
class GenTransactionTreeTest
  ...
  // Generates dummy subview references (ByViewHash or ByCiphertextId based on protocol version)
  // paired with a key (i.e. secure randomness) for each element.
  private def generateRandomKeysForSubviewHashes(
    ...
    subviewHashes.map(subviewHash =>
      SubviewReferenceAndKey(
        if (testedProtocolVersion < ProtocolVersion.transparency) ByViewHash(subviewHash)
        // for the purposes of this test, we use the view hash as a dummy ciphertext ID, as all view hashes in the
        // test are unique
        else ByCiphertextId(subviewHash.unwrap, NonNegativeInt.zero),
  ...
  private def lightTransactionViewTreeWithRandomKeys(
      ...
      allViewHashes: Seq[ViewHash],
      pureCrypto: CryptoPureApi,
  ): Either[String, LightTransactionViewTree] = {
    // we are not interested in the correctness of the subtree keys
    val randomnesses = generateRandomKeysForSubviewHashes(tvt.subviewHashes, pureCrypto)
    ...
    if (testedProtocolVersion < ProtocolVersion.transparency) {
      ...
      // This test verifies the correct validation and reconstruction of information within a forest of
      // LightTransactionViewTrees. The ByCiphertextId reference is used during the reconstruction of a
      // FullTransactionViewTree, but the actual ciphertext ID is not relevant for this test. We
      // assume that decryption is performed correctly and that the ciphertext ID uniquely identifies the
      // view. Consequently, for the purposes of this test, we use the view hash as a dummy ciphertext ID,
      // as all view hashes in the test are unique.
      val byCiphertextIdMap = allViewHashes
        .map(viewHash => viewHash -> ByCiphertextId(viewHash.unwrap, NonNegativeInt.zero))
        .toMap
      LightTransactionViewTree.fromTransactionViewTreeUsingCiphertextIdReference(
        tvt,
        randomnesses,
        byCiphertextIdMap,
        testedProtocolVersion,
  ...
  private def allLightTransactionViewTreesWithRandomKeys(
    ...
    allTransactionViewTrees
      .traverse(
        lightTransactionViewTreeWithRandomKeys(
          _,
          allTransactionViewTrees.map(_.viewHash),
          pureCrypto,
        )
      )
  ...
  // Attach the optional ciphertext ID lookup.
  // It is only done from the transparency protocol version onwards.
  private def attachCiphertextIdLookup(
      withViewPosition: Seq[(LightTransactionViewTree, ViewPosition)]
  ): Seq[((LightTransactionViewTree, ViewPosition), Option[ByCiphertextId])] =
    if (testedProtocolVersion < ProtocolVersion.transparency)
      withViewPosition.map(ltv => ltv -> None)
    else
      withViewPosition.map(ltv =>
        ltv -> Some(ByCiphertextId(ltv._1.viewHash.unwrap, NonNegativeInt.zero))
      )

  // Returns the root view trees in the sequence by filtering out any view
  // that is referenced as a subview by another view in the same sequence.
  private def topLevelViews(
      transaction: Seq[FullTransactionViewTree]
  ): Seq[FullTransactionViewTree] =
    transaction.filter(tree => !transaction.exists(_.subviewHashes.contains(tree.viewHash)))
    ...
    s"$example" can {
      ...
      "correctly reconstruct the full transaction view trees from the lightweight ones" in {
        ...
        LightTransactionViewTree.toFullViewTrees(
          ...
          testedProtocolVersion,
          factory.cryptoOps,
          attachCiphertextIdLookup(allLightTrees.map(ltv => withViewPosition(ltv))),
        ) shouldBe ToFullViewTreesResult(
          topLevelViews(allTrees).map(withViewPositions),
          Seq.empty,
          Seq.empty,
        )
      }
      ...
      "correctly reconstruct the top-level transaction view trees from the lightweight ones for each informee" in {
        ...
        val allLightTreesWithWitnesses: Seq[ViewWithWitnessesAndRecipients] =
          example.transactionTree
            .allTransactionViewTreesWithRecipients(topology)
            .valueOrFail("fail set up recipients for transaction view tree")
            .futureValueUS

        val allLightTrees: Seq[(LightTransactionViewTree, Witnesses)] =
          allLightTreesWithWitnesses.map {
            case ViewWithWitnessesAndRecipients(tvt, witnesses, _, _) =>
              lightTransactionViewTreeWithRandomKeys(
                tvt,
                allLightTreesWithWitnesses.map(_.view.viewHash),
                pureCrypto,
              )
        ...
        forAll(allInformees) { inf =>
          ...
          LightTransactionViewTree.toFullViewTrees(
            ...
            testedProtocolVersion,
            factory.cryptoOps,
            attachCiphertextIdLookup(allLightWeightForInf.map(ltv => withViewPosition(ltv))),
          ) shouldBe ToFullViewTreesResult(
      ...
      "correctly report missing subviews" in {
        ...
        // Picks one non-root view to remove
        ...
        // Computes which of the remaining views have the removed view as a descendant.
        ...
        // Computes the views that are expected to remain valid after removing a view.
        // Any view that has the removed view as a descendant should not be reconstructed,
        // as it depends on a missing child view.
        ...
        LightTransactionViewTree.toFullViewTrees(
          ...
          testedProtocolVersion,
          factory.cryptoOps,
          attachCiphertextIdLookup(inputLightTrees.map(ltv => withViewPosition(ltv))),
        ) shouldBe ToFullViewTreesResult(
          topLevelViews(expectedFullTrees).map(withViewPositions),
      ...
      "correctly process duplicate views" in {
        ...
        LightTransactionViewTree.toFullViewTrees(
          ...
          testedProtocolVersion,
          factory.cryptoOps,
          attachCiphertextIdLookup(inputLightTrees1.map(ltv => withViewPosition(ltv))),
        ) shouldBe ToFullViewTreesResult(
          topLevelViews(allFullTrees).map(withViewPositions),
          Seq.empty,
        ...
        LightTransactionViewTree.toFullViewTrees(
          ...
          testedProtocolVersion,
          factory.cryptoOps,
          attachCiphertextIdLookup(inputLightTrees2.map(ltv => withViewPosition(ltv))),
        ) shouldBe ToFullViewTreesResult(
          topLevelViews(allFullTrees).map(withViewPositions),
          Seq.empty,
          allLightTrees.map(withViewPosition),
  ...
  "A transaction tree" when {
    ...
    "a view and a subview have the same hash" must {
      "prevent creation" in {
        ...
        val subviews =
          TransactionSubviews(Seq(childView))(testedProtocolVersion, factory.cryptoOps)
        val parentView =
    ...
    forEvery(example.transactionViewTrees.zipWithIndex) { case (tvt, index) =>
      ...
      "given inconsistent subview hashes" must {
        s"reject creation (for the $index-th transaction view tree)" in {
          ...
          // For protocol versions that include transparency, subview references and their
          // respective keys are no longer tied to view hashes, but rather to ciphertext IDs.
          // Consequently, subview hashes are not included and, thus no longer validated, and the creation of the
          // light transaction view tree will succeed.
          if (testedProtocolVersion < ProtocolVersion.transparency)
