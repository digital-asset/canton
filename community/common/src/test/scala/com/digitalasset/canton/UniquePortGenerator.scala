...
object UniquePortGenerator {
  ...
  if (SharedPortNumFile.notExists) {
    mkdirs(SharedPortNumFile.parent)
    logger.debug(s"Initializing port file: ${SharedPortNumFile.path.toString}")
    SharedPortNumFile.write(PortRangeStart.toString)
  } else {
    logger.debug(s"Using exising port file: ${SharedPortNumFile.path.toString}")
  }
