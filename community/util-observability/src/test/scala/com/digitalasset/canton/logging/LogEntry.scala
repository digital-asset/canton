...
final case class LogEntry(
  ...
  override def toString: String = {
    val throwableStr = throwable match {
      case Some(t) => "\n" + ThrowableUtil.messageWithStacktrace(t)
      case None => ""
    }
    ...
    f"$level%-5s ${NamedLogging.readableQualifiedName(loggerName, 30)}$traceIdStr - $message$remainingMdcStr$throwableStr"
      .split('\n')
      .mkString("## ", "\n## \t", "")
  }
}
...
object LogEntry {
  ...
  def assertLogSeq(
    ...
    if (unexpectedLogEntries.nonEmpty) {
      val unexpectedEntriesStr = format(unexpectedLogEntries.map { case (entry, _) => entry })
      globalCheckpoint(
        fail(
    ...
    assertionsNotMatchingAnyLogEntry.foreach { case (clue, checkpoint) =>
      globalCheckpoint {
        withClue(s"\nDetails for assertion \"$clue\" without matching log entry:\n") {
          checkpoint.reportAll()
        } withClue "\n"
      }
    }
