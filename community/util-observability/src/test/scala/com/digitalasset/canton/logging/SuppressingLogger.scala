...
class SuppressingLogger private[logging] (
  ...
  val directExecutionContext: ExecutionContext = DirectExecutionContext(internalLogger)
    ...
    assertLogs(
      {
        ...
        assertion(t) withClue ThrowableUtil.messageWithStacktrace(t)
      },
  ...
  def checkLogsInternalError[T <: Throwable](
    ...
    entry.errorMessage shouldBe "An internal error has occurred."
    entry.throwable match {
      case Some(c(t)) => assertion(t) withClue ThrowableUtil.messageWithStacktrace(t)
  ...
  def checkLogsAssertion(assertion: Seq[LogEntry] => Assertion): Unit = {
    // check the log
  ...
  def runWithCleanup[T](body: => T, onSuccess: T => Unit, doFinally: () => Unit): T = {
    var isAsync = false
    try {
...
object SuppressingLogger {
  ...
  val skippedLogEntries: Seq[LogEntryCriterion] = Seq(
    LogEntryCriterion(
      WARN,
      "com.digitalasset.canton.concurrent.ExecutionContextMonitor",
