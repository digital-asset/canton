...
/** Enriches a [[com.digitalasset.canton.tracing.TraceContext]] with a [[NamedLoggerFactory]]. This
  * allows to pass a [[NamedLoggerFactory]] into methods that themselves do not have a
  * [[NamedLoggerFactory]] in scope on their own, e.g., because they live inside a static object or
  * in pure data classes.
  *
  * The logging methods of this class include the class's or object's name in the logger name. To
  * that end, the class or object should extend [[HasLoggerName]]. This ensures that the logger's
  * name in the log file is close to where the log message was actually created.
  *
  * @see
  *   NamedLogging.namedLoggingContext for implicitly creating this from the [[NamedLoggerFactory]]
  *   of the caller
  * @see
...
trait HasLoggerName extends Any {
  ...
  /** Convert a [[com.digitalasset.canton.logging.NamedLoggingContext]] into an
    * [[com.digitalasset.canton.logging.ErrorLoggingContext]] to fix the logger name to the current
    * class name.
    */
