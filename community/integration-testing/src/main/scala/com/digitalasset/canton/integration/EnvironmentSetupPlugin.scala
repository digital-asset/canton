...
/** Plugin to allow multiple tests to reuse behavior modifying the test config or environment
  * without having to modify the test definition. For instance running tests against Postgres vs the
  * default config, with the plugin being able to provide or verify the backend database exists.
  * Will work regardless of whether the test is using [[IsolatedEnvironments]] or
  ...
  */
trait BaseEnvironmentSetupPlugin[C <: SharedCantonConfig[C], E <: Environment[C]]
    extends NamedLogging {

  /** Run before any of the tests in the test class have been run or an environment created */
  def beforeTests(): Unit = {}

  /** Run after all of the tests in the test class have finished and the environment has been torn
    * down
    */
  def afterTests(): Unit = {}
