...
class DbSequencerStore(
  ...
  ): (Map[PayloadId, Batch[ClosedEnvelope]], Seq[PayloadId]) = {
    val preloadedPayloads = payloadIds.collect { case payload: BytesPayload =>
      payload.id -> payload.decodeBatch(protocolVersion)
    }.toMap
  ...
  override def readPayloadsByIdWithoutCacheLoading(
      ...
      attemptedLoadingFromCache <- FutureUnlessShutdown(
        ...
        Future
          .sequence(idsToLoad.map { id =>
            payloadCache.getIfPresent(id) match {
              case Some(value) => value.unwrap.map(_.map(payload => Right(id -> payload)))
              case None => Future.successful(UnlessShutdown.Outcome(Left(id)))
            }
          })
    ...
    } yield preloadedPayloads ++ (loadedFromDB ++ loadedFromCache.toMap).view
      .mapValues(_.decodeBatch(protocolVersion))
  }
  ...
  private def readEventsFromStore(
    ...
    def getResultFixedRecipients(
      ...
      GetResult { r =>
        val row = DeliverStoreEventRow[PayloadId](
          ...
          messageIdGetter(r),
          memberIdGetter(r),
          // instead of reading a large row, we construct a minimal recipients set here
          if (r.nextBoolean()) {
            Some(NonEmpty(SortedSet, memberId, topologyClientMemberId))
          } else {
            Some(NonEmpty(SortedSet, memberId))
          },
          payloadIdGetter(r),
          timestampOGetter(r),
          traceContextGetter(r).unwrap,
    ...
    def queryEventsViaRecipientsTable(
  ...
  private def readEventsLatest(
      ...
      traceContext: TraceContext
  ): FutureUnlessShutdown[Vector[Sequenced[IdOrPayload]]] = {
    def queryEvents(
