...
private[bftordering] final class P2PGrpcConnectionManager(
  ...
  // Note that we log gRPC channels with their native `toString` to be able to correlate them
  //  with orphaned warnings (leaks) from gRPC itself.
      ...
      openGrpcChannel(p2pEndpoint)
        .flatMap {
          case Some(
            ...
            p2pConnectionEventListener.onConnect(Some(p2pEndpointId))
    ...
    sequencerIdUS.futureUS
      ...
      .transformWith {
        case f @ Failure(exception) =>
          logger.info(
            s"Failed adding the P2P endpoint $p2pEndpointId, transitioning to disconnected and " +
              s"shutting down the gRPC channel $channel",
            exception,
          )
  ...
  private def openGrpcChannel(
    ...
    if (success) {
      ...
      logger.info(
        s"Shutting down gRPC channel $channelId with authentication context $authenticationContextId " +
  ...
  private def createPeerSender(
    ...
    OptionT(synchronizeWithClosing("p2p-create-peer-sender") {
      ...
      def retry(
          ...
          attemptNumber: Int,
      ): OptionT[FutureUnlessShutdown, PeerSender] = {
...
private[bftordering] object P2PGrpcConnectionManager {
          ...
          p2pConnectionsStatus.get(p2pEndpointId) match {
            case None =>
              ...
              State(
                ...
                Level.INFO -> (() => s"Disconnected (not in state) -> $newState"),
              )
            ...
            case Some(status) =>
              status match {
                ...
                case oldState @ P2POutgoingConnectionStatus.DisconnectingFromChannel(ch, acO, cw) =>
                  // Connect worker still active on a gRPC channel and asked to disconnect, cancel disconnect request
                  val newState = P2POutgoingConnectionStatus.ConnectingOnChannel(ch, acO, Some(cw))
                  State(
                    UnlessShutdown.Outcome(p2pConnectionsStatus.updated(p2pEndpointId, newState))
                  ) -> ResultWithLogs(false, Level.INFO -> (() => s"$oldState -> $newState"))
                ...
                case oldState @ P2POutgoingConnectionStatus.DisconnectingFromChannel(ch, _, _) =>
                  if (ch == channel)
                    ...
                    State(
                      ...
                      false,
                      Level.INFO -> (() => s"$oldState -> Disconnected (not in state)"),
                    )
                  else
                    this -> ResultWithLogs(false, Level.WARN -> (() => s"$oldState (unchanged)"))
              ...
              status match {
                ...
                case oldState @ P2POutgoingConnectionStatus.ConnectingOnChannel(ch, acO, _) =>
                  if (ch == channel) {
                    ...
                    State(
                      UnlessShutdown.Outcome(p2pConnectionsStatus.updated(p2pEndpointId, newState))
                    ) -> ResultWithLogs(None, Level.INFO -> (() => s"$oldState -> $newState"))
                  } else {
                    this -> ResultWithLogs(
                      Some(channel -> acO),
              ...
              p2pConnectionStatus match {
                ...
                case oldState @ P2POutgoingConnectionStatus.ConnectingOnChannel(
                  ...
                  State(
                    ...
                    Left(cw),
                    Level.INFO -> (() => s"$oldState -> $newState"),
                  )
  ...
  sealed trait P2POutgoingConnectionStatus
      extends Product
      with Serializable
  ...
  object P2POutgoingConnectionStatus {
    ...
    final case object Connecting extends P2POutgoingConnectionStatus {
      override def prettyCompanion: PrettyPrintingCompanion[Connecting.this.type] =
        ConnectingPrettyPrintingCompanion
    }

    private object ConnectingPrettyPrintingCompanion
        extends PrettyPrintingCompanion[Connecting.type] {
      override protected val pretty: Pretty[Connecting.type] = prettyOfObject[Connecting.type]
    }
    ...
    final case class ConnectingOnChannel(
      ...
      override def prettyCompanion: PrettyPrintingCompanion[ConnectingOnChannel] =
        ConnectingOnChannel
    }

    object ConnectingOnChannel extends PrettyPrintingCompanion[ConnectingOnChannel] {

      override protected val pretty: Pretty[ConnectingOnChannel] =
        prettyOfClass(
    ...
    final case class ConnectedOnChannel(
      ...
      override def prettyCompanion: PrettyPrintingCompanion[ConnectedOnChannel] =
        ConnectedOnChannel
    }

    object ConnectedOnChannel extends PrettyPrintingCompanion[ConnectedOnChannel] {

      override protected val pretty: Pretty[ConnectedOnChannel] =
        prettyOfClass(
          param("ch", _.channel.toString.unquoted),
          param("acO", _.authenticationContextO.map(objId)),
        )
    ...
    final case class DisconnectingFromChannel(
      ...
      override def prettyCompanion: PrettyPrintingCompanion[DisconnectingFromChannel] =
        DisconnectingFromChannel
    }

    object DisconnectingFromChannel extends PrettyPrintingCompanion[DisconnectingFromChannel] {

      override protected val pretty: Pretty[DisconnectingFromChannel] =
        prettyOfClass(
          param("ch", _.channel.toString.unquoted),
          param("cw", objId(_)),
          param("acO", _.authenticationContextO.map(objId)),
