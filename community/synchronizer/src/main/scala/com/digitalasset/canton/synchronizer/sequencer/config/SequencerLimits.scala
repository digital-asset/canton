...
/** Local limits for the sequencer public API.
  ...
  * @param maxMemberProtocolVersions
  *   maximum number of entries accepted in authentication challenge `memberProtocolVersions`
  */
final case class SequencerLimits(
    maxClientProtocolVersions: PositiveInt = PositiveInt.tryCreate(1000),
    maxMemberProtocolVersions: PositiveInt = PositiveInt.tryCreate(1000),
)
