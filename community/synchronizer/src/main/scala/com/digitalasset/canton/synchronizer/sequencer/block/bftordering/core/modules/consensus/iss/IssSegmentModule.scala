...
class IssSegmentModule[E <: Env[E]](
  ...
  override protected def receiveInternal(consensusMessage: ConsensusSegment.Message)(implicit
    ...
    consensusMessage match {
      ...
      case ConsensusSegment.ConsensusMessage.BlockOrdered(
            metadata,
            isEmpty,
            hasCompletedLedSegment,
          ) =>
        maybeOriginalLeaderSegmentState.fold {
          if (hasCompletedLedSegment) segmentLatencyMetric.recordSegmentLedByThisNodeCompleted()
          logger.debug(
            s"$messageType: not the original leader for this segment, not ordering a new block."
          )
        } { mySegmentState =>
...
object IssSegmentModule {
  ...
  private final case class RelativeSegmentLatencyMetric(
    ...
    def recordSegmentLedByThisNodeCompleted(): Unit = if (segmentLeadByThisNodeEndTime.isEmpty) {
      segmentLeadByThisNodeEndTime = Some(Instant.now())
      recordIfReady()
    }
