...
/** Signals that all updates in a block have been delivered as chunks. The
  * [[com.digitalasset.canton.synchronizer.block.data.BlockInfo]] must be consistent with the
  * updates in all earlier [[ChunkUpdate]]s. In particular:
...
/** Changes from processing a consecutive part of updates within a block from the blockchain. We
  * expect all values to be consistent with one another:
  *   - new members must exist in the registered members
  *   - the provided timestamps must be at or after the latest sequencer timestamp of the previous
  *     chunk or block
  *   - members receiving events must be registered
  *   - timestamps of events must not after the latest sequencer timestamp of the previous chunk or
  *     block
  ...
  * @param invalidAcknowledgements
  *   All invalid acknowledgement timestamps in the block for each member.
  * @param inFlightAggregationUpdates
  *   The updates to the in-flight aggregation states. Includes the clean-up of expired
  *   aggregations.
