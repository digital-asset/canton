      ...
      for {
        _ <- epochs.parTraverse { epoch =>
          for {
            ...
            _ <- storage
              .update(
                sqlu"""insert into ord_epochs(epoch_number, start_block_number, epoch_length, topology_ts, in_progress) values ($epoch, $epoch,10, $epoch, false) on conflict (epoch_number) do nothing""",
                "add block",
              )
              .failOnShutdown
          } yield ()
