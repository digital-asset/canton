...
class SortedReconciliationIntervalsProviderTest
  ...
  "SortedReconciliationIntervalsProvider" must {
    "allow to query reconciliation intervals" in {
      ...
      when(topologySnapshot.listDynamicSynchronizerParametersChanges())
        .thenAnswer(
          FutureUnlessShutdown.pure(synchronizerParameters.filter(_.validFrom <= clock.now))
        )
    ...
    "return an error if topology is not known" in {
      ...
      val error =
        s"Unable to query synchronizer parameters at $invalidQueryTime ; latest possible is $topologyKnownAt"
