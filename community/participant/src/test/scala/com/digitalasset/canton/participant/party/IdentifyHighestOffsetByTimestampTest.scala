...
/** Test the getHighestOffsetByTimestamp semantics of the offset computation from timestamp.
  */
class IdentifyHighestOffsetByTimestampTest extends AnyWordSpec with BaseTest {
  ...
  "offline party replication offset-from-timestamp computation" should {
    ...
    "error if the requested timestamp is not clean" in {
      val error = GrpcPartyManagementService
        ...
        .left
        .value
      error.code shouldBe PartyManagementServiceError.UnprocessedRequestedTimestamp.code
    }
      ...
      loggerFactory.suppressErrors({
        val error =
          GrpcPartyManagementService
            ...
            .left
            .value
        error shouldBe a[PartyManagementServiceError.InternalInvariantViolation.Error]
        ...
        val error =
          GrpcPartyManagementService
            .identifyHighestOffsetByTimestamp(
              requestedTimestamp = requestedTimestamp,
              synchronizerOffsetBeforeOrAtRequestedTimestamp =
                synchronizerOffsetLargerThanLedgerEnd,
              forceFlag = false,
              cleanSynchronizerTimestamp = timestampAfter,
              ledgerEnd = ledgerEnd,
              synchronizerId,
            ...
            .value
        error shouldBe a[PartyManagementServiceError.InternalInvariantViolation.Error]
        error.cause should include regex "The synchronizer offset .* exceeds the ledger end offset"
      })
    }
  }
}
