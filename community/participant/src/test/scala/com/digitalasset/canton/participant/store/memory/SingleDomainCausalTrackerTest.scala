...
class SingleDomainCausalTrackerTest
  ...
  private lazy val participant: ParticipantId = ParticipantId("p1-mydomain")

  private lazy val domain1: DomainId = DomainId.tryFromString("domain1::namespace")
  private lazy val sourceDomain1 = SourceDomainId(domain1)

  private lazy val domain2: DomainId = DomainId.tryFromString("domain2::namespace")
  private lazy val sourceDomain2 = SourceDomainId(domain2)
  private lazy val targetDomain2 = TargetDomainId(domain2)

  private lazy val domain3: DomainId = DomainId.tryFromString("domain3::namespace")

  private lazy val domain4: DomainId = DomainId.tryFromString("domain4::namespace")
  private lazy val sourceDomain4 = SourceDomainId(domain4)
  private lazy val targetDomain4 = TargetDomainId(domain4)
    ...
    "have no causal dependencies" in {
      ...
      val sut =
        new SingleDomainCausalTracker(
          gco,
          new InMemorySingleDomainCausalDependencyStore(sourceDomain1, loggerFactory),
          loggerFactory,
        )
  ...
  "in a simple-two domain topology, events" should {
    "maintain causal per-party dependencies across domains" in {
      ...
      val sut =
        new SingleDomainCausalTracker(
          gco,
          new InMemorySingleDomainCausalDependencyStore(sourceDomain2, loggerFactory),
          loggerFactory,
        )

      val id = TransferId(
        sourceDomain = sourceDomain1,
        requestTimestamp = CantonTimestamp.Epoch.minusSeconds(1),
      )
      ...
      val cm = CausalityMessage(
        targetDomain2,
        testedProtocolVersion,
        id,
        VectorClock(sourceDomain1, localTs = outTime, alice, Map(domain1 -> outTime)),
      )
      ...
      val expectedEvents = updates
        .foldLeft(List.empty[EventPerPartyCausalState]) { case (eventsList, update) =>
          val delta: Map[DomainId, CantonTimestamp] = update match {
            case _: TransactionUpdate => Map.empty
            // Causal dependencies should be introduced by the transfer-in
            case update: TransferInUpdate => Map(update.transferId.sourceDomain.unwrap -> outTime)
            case _ => fail()
          }
    ...
    "register causality information on transfer out" in {
      ...
      val id = TransferId(sourceDomain = sourceDomain2, requestTimestamp = outTime)
      ...
      transferInfo shouldBe Map(
        alice -> VectorClock(sourceDomain2, outTime, alice, clock = Map(domain2 -> outTime))
      )
    }
  }
  ...
  "in more complex cases, events" should {
    ...
    "save only per-party causal state" in {
      ...
      val sut =
        new SingleDomainCausalTracker(
          gco,
          new InMemorySingleDomainCausalDependencyStore(sourceDomain4, loggerFactory),
          loggerFactory,
        )

      // First transfer from d1 to d4
      val outTime = CantonTimestamp.Epoch.plusSeconds(5)
      val transfer1ID = TransferId(sourceDomain = sourceDomain1, requestTimestamp = outTime)

      // Second transfer out from d4
      val outTime2 = CantonTimestamp.Epoch.plusSeconds(10)
      val transfer2ID = TransferId(sourceDomain = sourceDomain4, requestTimestamp = outTime2)
      ...
      val expectedEvents = updates
        .foldLeft(
          ...
          val delta: Map[LfPartyId, Map[DomainId, CantonTimestamp]] = causalityUpdate match {
            ...
            case TransferInUpdate(_parties, ts, TargetDomainId(targetDomain), _rc, _transferId) =>
              Map(
                alice -> Map(domain1 -> outTime, domain3 -> aliceD3Time, targetDomain -> ts),
                bob -> Map(domain1 -> outTime, domain2 -> bobD2Time, targetDomain -> ts),
              )

            case TransferOutUpdate(parties, ts, transferId, _rc) =>
              (parties.toList.map(p => p -> Map(transferId.sourceDomain.unwrap -> ts)).toMap)
