...
final class AssignmentProcessingStepsTest
  ...
  "prepare submission" should {
    ...
    "fail when target synchronizer has different lsid" in {
      ...
      val upgradedTargetPsid = targetPsid.map(_.incrementSerial.value)
    ...
    "succeed without errors" in {
      ResourceUtil.withResourceM(
        ...
        for {
          ...
          decryptedView = decrypted.views.loneElement
          view = decryptedView.view
          activenessSet =
            assignmentProcessingSteps
              .computeActivenessSet(
                mkParsedRequest(
                  view.unwrap,
                  view.recipients,
                ).copy(signatureO = decryptedView.signatureO)
              )
              .value
        } yield {
