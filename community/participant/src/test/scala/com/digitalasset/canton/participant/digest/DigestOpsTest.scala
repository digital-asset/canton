...
class DigestOpsTest extends AnyWordSpec with BaseTest {
  ...
  "Calculating a single digest" should {
    "return the expected result in a simple scenario" in {
      ...
      digest.hexString() shouldBe expectedDigestHexString
    }
  }

  "Calculating multiple digests" should {
    "return the same result regardless of the order" in {
      ...
      combined1.hexString() shouldBe combined2.hexString()
    }
  }

  "Calculating the digest delta" should {
    ...
    "return the same result for the counter-participant" in {
      ...
      val deltaForParticipant2Opt = DigestOps
        .computeDeltas(
          participant1,
          AcsUpdate(
            stakeholders = Map(
      ...
      val deltaForParticipant1Opt = DigestOps
        .computeDeltas(
          participant2,
          AcsUpdate(
            stakeholders = Map(
      ...
      inside((deltaForParticipant2Opt, deltaForParticipant1Opt)) {
        case (Some(deltaForParticipant2), Some(deltaForParticipant1)) =>
          deltaForParticipant2.digest shouldBe deltaForParticipant1.digest
      }
    }
  }
