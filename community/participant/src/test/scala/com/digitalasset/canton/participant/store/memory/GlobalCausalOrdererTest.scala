...
class GlobalCausalOrdererTest extends AnyWordSpec with BaseTest with HasExecutionContext {
  ...
  "basic ordering of two events" in {
    ...
    val canPublish1F = sut.waitPublishable(clock1.clock).failOnShutdown
    val canPublish2F = sut.waitPublishable(clock2.clock).failOnShutdown
  ...
  "ordering where an event has multiple causal dependencies on different domains" in {
    ...
    val canPublish1F = sut.waitPublishable(clock1.clock).failOnShutdown
    val canPublish2F = sut.waitPublishable(clock2.clock).failOnShutdown
    val canPublish4F = sut.waitPublishable(clock4.clock).failOnShutdown
  ...
  "allow out-of-order publishing on a single domain" in {
    ...
    val canPublish1F = sut.waitPublishable(clock1.clock).failOnShutdown
    val canPublish2F = sut.waitPublishable(clock2.clock).failOnShutdown
    val canPublish3F = sut.waitPublishable(clock3.clock).failOnShutdown
  ...
  "transfer-ins block on transfer-outs" in {
    ...
    val sourceDomain = SourceDomainId(domain1)
    val id = TransferId(sourceDomain = sourceDomain, CantonTimestamp.Epoch)
    val party = LfPartyId.assertFromString("Alice::domain")
    val vc = VectorClock(sourceDomain, CantonTimestamp.Epoch, party, Map.empty)
    ...
    map shouldBe Map(party -> VectorClock(sourceDomain, CantonTimestamp.Epoch, party, Map.empty))
  }
}
