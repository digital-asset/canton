...
class RecordOrderPublisherTest extends AnyWordSpec with BaseTest with HasExecutionContext {
  ...
  "the RecordOrderPublisher event recovery" should {
    ...
    def createSut(domain: DomainId): Fixture = {
      ...
      val causalStore =
        new InMemorySingleDomainCausalDependencyStore(SourceDomainId(domain), loggerFactory)
    ...
    val id = TransferId(SourceDomainId(domain1), domain1Ts1)
          ...
          List(
            CausalityMessage(
              ...
              VectorClock(
                ...
                id.requestTimestamp,
                alice,
                Map(id.sourceDomain.unwrap -> id.requestTimestamp),
              ),
            )
          )
        )
        .futureValue
      gco.registerPublished(EventClock(id.sourceDomain.unwrap, id.requestTimestamp, 0L)(Map.empty))
