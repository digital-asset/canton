...
/** Factory class that can create the [[com.digitalasset.canton.data.GenTransactionTree]]s from a
  ...
  */
class LegacyTransactionTreeFactory(
  ...
  private def createRootViews(
    ...
    def go(view: TransactionViewDecomposition.NewView): Unit = {
      ...
      preloadCids.addAll(view.tailNodes.flatMap(x => LfTransactionUtil.usedContractId(x.lfNode)))
        ...
        case childView: TransactionViewDecomposition.NewView =>
          ...
          createView(
            ...
            .map { v =>
              ...
              val keyResolutionsInSubview = v.legacyGlobalKeyInputs.fmap(_.map(_.asSerializable))
  ...
  private def checkCsmStateMatchesView(
    ...
    val viewGki = transactionView.legacyGlobalKeyInputs.fmap(_.unversioned.resolution)
...
object LegacyTransactionTreeFactory {
