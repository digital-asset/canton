...
/** Configuration for a single engine extension service.
  ...
  * @param tlsInsecure
  *   If true, skip TLS certificate validation (dev only)
  * @param jwt
  *   JWT token for authentication
  * @param jwtFile
  *   Path to file containing JWT token
  * @param connectTimeout
  *   Connection timeout
  * @param requestTimeout
  *   Request timeout for individual HTTP requests
  * @param maxTotalTimeout
  *   Maximum total time for the entire operation including retries
  * @param maxRetries
  ...
  *   Functions that this extension provides, for validation
  */
...
/** Declaration of a function provided by an extension service.
  ...
  * @param configHash
  *   Expected configuration hash for version validation
  */
...
/** Configuration for engine extensions in the participant.
  *
  * @param validateExtensionsOnStartup
  *   Whether to validate extension configurations on startup
  * @param failOnExtensionValidationError
  *   Whether to fail startup if extension validation fails
  * @param echoMode
  *   If true, external calls return the input as output (for testing)
  */
