...
class DbAcsCommitmentPeriodStore(
  ...
  private def lookupInTable[Digest, Off](
    ...
    if (periods.isEmpty) FutureUnlessShutdown.pure(immutable.Iterable.empty)
      ...
      val participants = toIntArrayLiteral(periods.iterator.map { case (participant, _) =>
        participant
      })
      val fromExclusives = toLongArrayLiteral(periods.iterator.map { case (_, period) =>
        period.fromExclusive.toMicros
      })
      val toInclusives = toLongArrayLiteral(periods.iterator.map { case (_, period) =>
  ...
  override def purge()(implicit traceContext: TraceContext): FutureUnlessShutdown[Unit] = {
    val purges =
      DBIO.seq(
        sqlu"delete from par_acs_commitment_period_outstanding where synchronizer_idx = $indexedSynchronizer",
        sqlu"delete from par_acs_commitment_period_mismatch where synchronizer_idx = $indexedSynchronizer",
        sqlu"delete from par_acs_commitment_period_match where synchronizer_idx = $indexedSynchronizer",
        sqlu"delete from par_acs_commitment_period_watermark where synchronizer_idx = $indexedSynchronizer",
      )
    storage.update_(purges, functionFullName)
  }
