...
final class InMemoryAcsCommitmentPeriodStore(
  ...
  override def purge()(implicit traceContext: TraceContext): FutureUnlessShutdown[Unit] = {
    withLock {
      ...
      mismatched.clear()
      matched.clear()
      watermarksVar = MatchingWatermark.initial
    }
    FutureUnlessShutdown.unit
  }
  ...
  private def overlaps[Digest, Off, Digest2, Off2](map: Table[Digest, Off])(
    ...
    val toInclusive = period.toInclusive
    val toExclusive =
      Option.when(toInclusive < CantonTimestamp.MaxValue)(toInclusive.immediateSuccessor)
    map.get(participant).toList.flatMap { periodMap =>
      val others = periodMap
        .rangeImpl(Some(period.fromExclusive.immediateSuccessor), toExclusive)
        .valuesIterator
        .flatMap(mapFilter)
      val overlapsRight =
        if (period.toInclusive < CantonTimestamp.MaxValue)
