  ...
  private def signalResultToRequestTracker(
    ...
    for {
      ...
      commitFuture <- EitherT
        ...
        .leftMap(e => {
          SyncServiceAlarm.Warn(s"Failed to add commit set for $requestId. $e").report()
          e: RequestTracker.RequestTrackerError
        })
    } yield commitFuture
      .valueOr(irregularities =>
        SyncServiceAlarm
          .Warn(withRc(rc, s"Result message causes ACS irregularities: $irregularities"))
          .report()
      )
