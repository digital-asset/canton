  ...
  private def assignClocksAndStore(
    ...
    val writeF: Future[CausalityWrite] = update match {
      ...
      case transferOut: TransferOutUpdate =>
        ...
        val sourceDomain = transferOut.sourceDomain
        val write = updatePartyClocksAndGet(
          parties,
          sourceDomain.unwrap,
          transferOut.ts,
          Some(transferOut.transferId),
        )
        ...
        for {
          ...
          val vectorClocks =
            clocks.stateAtWrite.map { case (partyId, clock) =>
              VectorClock(sourceDomain, transferOut.ts, partyId, clock)
            }.toSet
      ...
      case TransferInUpdate(parties, ts, domain, rc, transferId) =>
        awaitAndFetchTransferOut(transferId, parties).flatMap { vectorClocksAtTransferOut =>
          ...
          val delta = vectorClocksAtTransferOut.map { case (pid, vc) =>
            ...
            val pidDelta = clockAtTransferOut + (domain.unwrap -> ts)
            pid -> pidDelta
          }
