...
class AcsCommitmentSender(
  ...
  def startPipeline(
    ...
    // This doesn't use `futureSourceUS`, because the graph will potentially be materialized multiple times,
    // and the crash recovery checkpoint should be determined from scratch. `futureSourceUS` would reuse the
    ...
    def restartSendLoop(): Unit =
      synchronizeWithClosingSync("start ACS commitment sender loop") {
        val handle @ (ks, doneF) =
          ...
          .onComplete {
            case Success(_) =>
              if (isClosing) {
                ...
                logger.info(
                  "The send-loop terminated orderly and will be restarted."
                )
                restartSendLoop()
              }
            case Failure(ex) =>
              logger.info(
                "The send-loop has failed with an error and will be restarted.",
                ex,
              )
              restartSendLoop()
  ...
  def sendAcsCommitments(
    ...
    sendAcsCommitmentBatchesRecursively(
      ...
      watermarkStore.increaseWatermark(timepoint)
    }
