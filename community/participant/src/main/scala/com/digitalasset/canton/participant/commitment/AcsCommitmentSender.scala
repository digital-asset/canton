...
class AcsCommitmentSender(
  ...
  private def makeInternalDigests(
      digestUpdates: Seq[AcsDigestUpdate[InternedParticipantId]]
    ...
    digestUpdates.view
      ...
      .collect { digest =>
        digest.digestO match {
          case Some(value) =>
            val hashedDigest = Digest.hashDigest(value).getCryptographicEvidence
            InternalAcsDigest(
              key = digest.key,
...
object AcsCommitmentSender {
  ...
  private object AcsCommitmentSenderError {
    ...
    case object UnexpectedSendResultError extends AcsCommitmentSenderError
  }
}
