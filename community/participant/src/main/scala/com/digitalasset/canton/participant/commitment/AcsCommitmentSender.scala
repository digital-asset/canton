...
class AcsCommitmentSender(
  ...
  def sendAcsCommitments(
    ...
    ).semiflatMap { batchCount =>
      metrics.sentBatchCount.inc(batchCount.value.toLong)
  ...
  /** The B type of the returned EitherT is either:
    ...
    *   - The number of batches sent if it's the final batch
    */
  private def sendSingleBatch(
    ...
    val nextStepResult = batchSendingResult
      .map[Either[RecursionStep[digestJournal.SnapshotPaginationToken], NonNegativeInt]] { result =>
        result.paginationResult match {
          case Left(PaginationTokenDone) =>
            Right(result.nextBatchIndex)
  ...
  /** The B type of the returned EitherT is the number of batches sent.
    */
  private def sendAcsCommitmentBatchesRecursively(
      ...
      tc: TraceContext
  ): EitherT[FutureUnlessShutdown, AcsCommitmentSenderError, NonNegativeInt] =
    Monad[EitherT[FutureUnlessShutdown, AcsCommitmentSenderError, *]]
      .tailRecM[RecursionStep[digestJournal.SnapshotPaginationToken], NonNegativeInt](
