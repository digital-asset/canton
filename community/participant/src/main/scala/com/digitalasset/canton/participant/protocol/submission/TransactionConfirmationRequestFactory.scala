...
class TransactionConfirmationRequestFactory(
  ...
  private def createTransactionViewEnvelopes(
    ...
    def createOpenEnvelopesWithTransactionV2(
      ...
      def processGroupOfRecipients(
        ...
        for {
          ...
          updates = lightTrees.zipWithIndex.map { case (lvt, i) =>
            ...
            lvt.viewHash -> ByCiphertextId(
              ...
              index = PositiveInt.tryCreate(i),
            )
          }

          // Update state with the new ciphertext IDs and the new envelope
          updatedMap = state.byCiphertextIdMap ++ updates
          updatedEnvelopes =
            state.envelopes :+ OpenEnvelope(encrypted, recipients)(protocolVersion)

        } yield ViewEncryptionAccumulator(updatedMap, updatedEnvelopes)
      }

      if (!viewsWithWitnessesAndRecipients.forall(_.recipients.trees.sizeIs == 1))
        ErrorUtil.invalidState("Expected all views to have exactly one recipient tree in Phase 1")

      // Group views by recipient tree size, largest first (i.e. post-order), to ensure that when encrypting a view with
      ...
      // recipient tree size can't reference each other.
      ...
      MonadUtil
        .foldLeftM(
          ...
          groupedBySize,
        ) { case (acc, (_, groupsOfSameSize)) =>
          // Process all groups (including encryption) of the same size in parallel
          MonadUtil
            .parTraverseWithLimit(pureCrypto.encryptionParallelism)(groupsOfSameSize.toNEF) {
              group =>
                processGroupOfRecipients(acc, group)
            }
            .map { results =>
              // Merge all results produced in parallel
              results.foldLeft(acc) { (merged, next) =>
                ViewEncryptionAccumulator(
                  merged.byCiphertextIdMap ++ next.byCiphertextIdMap,
                  merged.envelopes ++ next.envelopes,
                )
              }
            }
        }
    ...
    for {
      ...
      envelopes <- createOpenEnvelopesWithTransactionV1(
