...
class CommitmentSenderMetrics private[metrics] (
  ...
  val sentCommitmentCount: MetricHandle.Counter = metricsFactory.counter(
    MetricInfo(
      prefix :+ "sent-commitment-count",
      summary = "Number of sent ACS commitments",
      description =
        """In the happy scenario, the value should increase at each tick proportional to the number of counterparticipants.
          |It can increase more in case of crashes/unhandler errors due to retries.""".stripMargin,
      qualification = MetricQualification.Traffic,
    )
  )
