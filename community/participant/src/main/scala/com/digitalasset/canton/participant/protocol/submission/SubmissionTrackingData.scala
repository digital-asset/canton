...
trait SubmissionTrackingData
  ...
  /** Update the tracking data so that the deliver error [[com.google.rpc.status.Status]] can be
    ...
    * @param trafficCost
    *   The traffic cost incurred by the ordering of the event
    */
  def updateOnNotSequenced(
...
object SubmissionTrackingData
  ...
  val versioningTable: VersioningTable = VersioningTable(
    ProtoVersion(30) -> VersionedProtoCodec
      .storage(ReleaseProtocolVersion(ProtocolVersion.v35), v30.SubmissionTrackingData)(
...
final case class TransactionSubmissionTrackingData(
  ...
  override def updateOnNotSequenced(
      timestamp: CantonTimestamp,
      reason: Status,
      trafficCost: NonNegativeLong,
  )(implicit
    ...
    UnsequencedSubmission(
      timestamp,
      this.copy(
        completionInfo = completionInfo.copy(paidTrafficCost = trafficCost),
        rejectionCause = TransactionSubmissionTrackingData.CauseWithTemplate(reason),
      ),
    ).some
...
object TransactionSubmissionTrackingData
  ...
  def fromProtoV30(
    ...
    val v30.TransactionSubmissionTrackingData(
      completionInfoP,
      causeP,
      synchronizerIdP,
      transactionHashP,
