...
/** HTTP client implementation for extension services with retry logic, JWT authentication, and TLS
  * support.
  *
  * @param extensionId
...
class HttpExtensionServiceClient(
  ...
  private val endpoint: URI =
    URI.create(s"$scheme://${config.host}:${config.port}/api/${config.version}/external-call")
  ...
  override def validateConfiguration()(implicit
      tc: TraceContext
          ...
          try {
            ...
            if (resp.statusCode() >= 200 && resp.statusCode() < 600) {
              ...
              ExtensionValidationResult.Invalid(
                Seq(s"Unexpected response code: ${resp.statusCode()}")
              )
            }
          } catch {
            case e: java.net.ConnectException =>
              ExtensionValidationResult.Invalid(
                Seq(s"Cannot connect to extension service: ${e.getMessage}")
              )
  ...
  private def callWithRetry(
    ...
    def loop(
        attempt: Int,
        lastError: Option[ExtensionCallError],
      ...
      if (nowMs >= deadlineMs) {
        ...
        result match {
          case Right(response) =>
            if (attempt > 0) {
              logger.info(
                s"External call to extension '$extensionId' succeeded after $attempt retries"
              )
            }
            Right(response)
          ...
          case Left(error) =>
            logger.error(
              s"External call to extension '$extensionId' failed with non-retryable error: ${error.message} (status=${error.statusCode}, requestId=${error.requestId
                  .getOrElse("none")})"
            )
            Left(error)
        }
  ...
  private def singleCall(
    ...
    try {
      ...
      case e: java.net.ConnectException =>
        logger.error(
          s"External call to extension '$extensionId' connection failed: requestId=$requestId, error=${e.getMessage}"
        )
        Left(ExtensionCallError(503, s"Connection failed: ${e.getMessage}", Some(requestId)))

      case e: java.io.IOException =>
        logger.error(
          s"External call to extension '$extensionId' I/O error: requestId=$requestId, error=${e.getMessage}"
        )
        Left(ExtensionCallError(503, s"I/O error: ${e.getMessage}", Some(requestId)))

      case e: Exception =>
        logger.error(
          s"External call to extension '$extensionId' unexpected error: requestId=$requestId, error=${e.getMessage}"
        )
