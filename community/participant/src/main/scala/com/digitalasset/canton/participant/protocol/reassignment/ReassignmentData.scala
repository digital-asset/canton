...
/** Helper trait to make some data handling more generic.
  */
sealed trait ReassignmentData {
  def reassignmentId: ReassignmentId
  def stakeholders: Set[LfPartyId]
}

object ReassignmentData {
  final case class IncompleteReassignment(
  ...
  final case class IdAndStakeholders(reassignmentId: ReassignmentId, stakeholders: Set[LfPartyId])
      extends ReassignmentData

  object IdAndStakeholders {
    implicit def reassignmnetDataGetResult(implicit
        getResultStakeholders: GetResult[NonEmpty[Set[LfPartyId]]]
    ): GetResult[IdAndStakeholders] = GetResult { r =>
      val reassignmentId = GetResult[ReassignmentId].apply(r)
      val stakeholders = GetResult[NonEmpty[Set[LfPartyId]]].apply(r)

      IdAndStakeholders(reassignmentId, stakeholders)
    }
  }
}
