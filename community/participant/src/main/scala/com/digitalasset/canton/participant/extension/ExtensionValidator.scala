...
/** Validates that DAR extension requirements are satisfied by configured extensions.
  *
  * This validator is called on participant startup to ensure that all DARs have their extension
  * requirements satisfied by the participant's configuration.
  *
  * @param extensionManager
  *   The extension service manager
  * @param loggerFactory
  *   Logger factory
  */
class ExtensionValidator(
  ...
  def validateExtensionRequirement(
      ...
      functionId: String,
      configHash: String,
  ): ExtensionRequirementValidation =
  ...
  def logValidationResults(
    ...
    val errors: Seq[ExtensionRequirementValidation] = results.collect {
      case e: ExtensionRequirementValidation.MissingExtension => e
      case e: ExtensionRequirementValidation.ConfigHashMismatch => e
    }

    val warnings: Seq[ExtensionRequirementValidation] = results.collect {
