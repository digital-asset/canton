...
/** Manages extension service connections with pooled HTTP clients.
  ...
  * @param engineExtensionsConfig
  *   Engine extensions configuration
  * @param loggerFactory
  *   Logger factory
  * @param ec
  *   Execution context
...
class ExtensionServiceManager(
  ...
  private val httpClient: HttpClient = {
    val builder = HttpClient
      ...
      ) // Global connect timeout, individual requests can override
  ...
  private val clients: Map[String, ExtensionServiceClient] =
    if (engineExtensionsConfig.echoMode) {
      logger.info(
        "Extension services running in echo mode - external calls will return input as output"
      )(TraceContext.empty)
  ...
  /** Handle an external call question from the engine.
    ...
    * @param commandId
    *   Command ID from the submitting client
    * @return
        ...
        FutureUnlessShutdown.pure(
          Left(
            ExtensionCallError(
              statusCode = 404,
              message =
                s"Extension '$extensionId' not configured. Available extensions: ${clients.keys.mkString(", ")}",
              requestId = None,
            )
          )
        )
  ...
  /** Validate all configured extensions on startup.
    *
    * @return
    *   Map of extension ID to validation result
    */
  def validateAllExtensions()(implicit
      tc: TraceContext
  ): FutureUnlessShutdown[Map[String, ExtensionValidationResult]] =
...
/** Echo extension service client for testing. Returns the input as the output without making any
  * HTTP calls.
  */
