...
import com.digitalasset.canton.participant.admin.data.ActiveContract.InvalidActiveContract
...
final case class ActiveContract private (
  ...
  if (protocolVersion >= ProtocolVersion.dev && transferCounter.isEmpty)
    throw new InvalidActiveContract(
      s"The reassignment counter must be defined for protocol version '${ProtocolVersion.dev}' or later"
    )
  if (protocolVersion < ProtocolVersion.dev && transferCounter.isDefined)
    throw new InvalidActiveContract(
      s"The reassignment counter must be undefined for protocol version earlier than '${ProtocolVersion.dev}'"
    )
  ...
  private def toProtoV1: v1.ActiveContract = {
    v1.ActiveContract(
      ...
      transferCounter
        .getOrElse(
          throw new IllegalStateException("Reassignment counter is required but was absent")
        )
        .toProtoPrimitive,
    )
  }
...
private[canton] object ActiveContract extends HasProtocolVersionedCompanion[ActiveContract] {
  ...
  def create(
    ...
    Either.catchOnly[InvalidActiveContract](
      ActiveContract(
        domainId: DomainId,
        contract: SerializableContract,
        transferCounter: TransferCounterO,
