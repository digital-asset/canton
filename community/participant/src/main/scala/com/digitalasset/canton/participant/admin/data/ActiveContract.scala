...
object ActiveContract extends VersioningCompanion[ActiveContract] {
  ...
  override val versioningTable: VersioningTable = VersioningTable(
    ProtoVersion(30) -> VersionedProtoCodec(ProtocolVersion.v35)(v30.ActiveContract)(
  ...
  def fromFile(
      fileInput: File,
      maxBytesToDecompress: Long,
  ): Either[Throwable, Iterator[ActiveContract]] =
    ResourceUtil.withResourceEither(
      ByteStringUtil.boundedInputStream(
        new GzipCompressorInputStream(fileInput.newFileInputStream),
        maxBytesToDecompress,
      )
    ) { stream =>
      loadFromSource(stream) match {
