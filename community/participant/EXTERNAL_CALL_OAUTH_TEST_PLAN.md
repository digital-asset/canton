...
- [x] Preserve participant startup with OAuth configured even when OAuth key or trust material is invalid until the first OAuth call path is exercised.
- [x] Do not send token-endpoint HTTP interactions during participant startup or extension-manager construction.
- [x] Preserve existing startup validation semantics without introducing OAuth-specific startup gating.
