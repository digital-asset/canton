...
- [x] Invalidate a cached token after a resource-server bearer `invalid_token` `401` only when that same token was sent.
...
- [x] Trigger auth-local invalidate-and-replay only on resource `401` with `WWW-Authenticate: Bearer error="invalid_token"`.
- [x] Do not trigger auth-local replay on resource `401` without `WWW-Authenticate`.
- [x] Do not trigger auth-local replay on resource `401` with a non-Bearer challenge.
- [x] Do not trigger auth-local replay on resource `401` with a Bearer challenge that does not include `error="invalid_token"`.
...
- [x] Serialize concurrent refreshes caused by resource-server bearer `invalid_token` `401`.
...
- [x] After bearer `invalid_token` replay exhaustion, map resource token rejection to `401` with message `Unauthorized - OAuth token rejected by resource server`.
...
- [x] Log token invalidation after resource-server bearer `invalid_token` `401`.
...
- [x] Cover single bearer `invalid_token` `401` refresh-and-replay.
...
- 2026-04-01: `HttpExtensionServiceClientOAuthTest` now also covers the originally implemented section 6 and section 9 replay behavior for single `401` invalidate-refresh-replay, replay exhaustion with the OAuth-specific terminal `401` message, and reuse of the freshly acquired token after a successful replay. The explicit red-green command remained `sbt 'community-participant/testOnly com.digitalasset.canton.participant.extension.HttpExtensionServiceClientOAuthTest'`: the new examples first failed against the pre-replay service-client path, then passed after adding the auth-local replay flow. This historical note predates the later 2026-04-02 narrowing that restricted replay to `WWW-Authenticate: Bearer error="invalid_token"`.
...
- 2026-04-02: `OAuthExternalCallIntegrationTest` now also covers a single resource-server `401` causing one token refresh and one exact replay. The explicit command remained `sbt 'community-app/testOnly com.digitalasset.canton.integration.tests.externalcall.OAuthExternalCallIntegrationTestH2'`, and this slice went green immediately once it was isolated on its own extension ID: one business request produced `2` token calls and `3` resource calls (`401`, replay success, validation success). This historical note predates the later 2026-04-02 narrowing that required the replay-triggering `401` to carry `WWW-Authenticate: Bearer error="invalid_token"`.
...
- 2026-04-02: `HttpExtensionServiceClientOAuthTest` now also narrows the replay trigger in sections 6 through 9 from any raw resource `401` to the explicit bearer challenge case `WWW-Authenticate: Bearer error="invalid_token"`, and it covers the new terminal no-replay cases for missing `WWW-Authenticate`, non-Bearer challenges, plain `Bearer`, and `Bearer error="insufficient_scope"`. The focused verification command was `sbt 'community-participant/testOnly com.digitalasset.canton.participant.extension.HttpExtensionServiceClientOAuthTest'`, which passed after `HttpExtensionServiceClient` stopped replaying on challenge-blind `401` responses.
- 2026-04-02: `OAuthExternalCallIntegrationTest` now aligns the replay fixture in section 12 with the same narrowed contract by returning `WWW-Authenticate: Bearer error="invalid_token"` on the first resource `401`. The focused verification command was `sbt 'community-app/testOnly com.digitalasset.canton.integration.tests.externalcall.OAuthExternalCallIntegrationTestH2 -- -z "refresh once and replay the resource request after a single resource-server 401"'`, which passed once the mock response emitted the explicit bearer challenge.
