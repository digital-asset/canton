...
class NonStakeholderInformeesITSpec
  ...
  "NonStakeholderInformees" should {
    "divulge events" onlyRunWithOrGreaterThan ProtocolVersion.v35 in { env =>
      import env.*
      ...
      val (apiServices, allocatedParties, fooSubmission) = benchtoolFooSubmissionFixture(
        submissionConfig
      ).futureValue
      fooSubmission.performSubmission(submissionConfig = submissionConfig).futureValue
      val (ledgerEffectsResults_divulgee0, acsDeltaResults_divulgee0) = observeAllTemplatesForParty(
        party = allocatedParties.divulgees.head,
        apiServices = apiServices,
        expectedTemplateNames = expectedTemplateNames,
      ).futureValue
      val (ledgerEffectsResults_divulgee1, acsDeltaResults_divulgee1) = observeAllTemplatesForParty(
        party = allocatedParties.divulgees(1),
        apiServices = apiServices,
        expectedTemplateNames = expectedTemplateNames,
      ).futureValue
      val (ledgerEffectsResults_observer0, acsDeltaResults_observer0) = observeAllTemplatesForParty(
        party = allocatedParties.observers.head,
        apiServices = apiServices,
        expectedTemplateNames = expectedTemplateNames,
      ).futureValue
      val (ledgerEffectsResults_signatory, _) = observeAllTemplatesForParty(
