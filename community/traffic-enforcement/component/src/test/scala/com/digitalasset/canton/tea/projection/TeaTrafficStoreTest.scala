  ...
  def teaTrafficStore(mk: () => TeaTrafficStore): Unit = {
    "TeaTrafficStore" should {
      ...
      "insert events in the store" in {
        ...
        for {
          afterPersist1 <- store.persist(alice, creditBalanceDelta(10), timestamp1)
          getBalance1 <- store.balance(alice)
          afterPersist2 <- store.persist(alice, debitBalanceDelta(5), timestamp2)
          getBalance2 <- store.balance(alice)
          afterPersist3 <- store.persist(alice, debitBalanceDelta(3), timestamp3)
      ...
      "accumulate credits and debits and expose the balance" in {
        ...
        val timestamp3 = timestamp2.immediateSuccessor
        for {
          _ <- store.persist(alice, creditBalanceDelta(100), timestamp1)
          _ <- store.persist(alice, debitBalanceDelta(30), timestamp2)
          _ <- store.persist(alice, debitBalanceDelta(20), timestamp3)
      ...
      "not overflow the total credit value" in {
        val store = mk()
        for {
          accountFetch <- store.persist(alice, creditBalanceDelta(Long.MaxValue), clock.now)
          // Going 1 above Long.MaxValue should fail
          error <- store.persist(alice, creditBalanceDelta(1L), clock.now).failed
        } yield {
          accountFetch.value.totalCredits.value shouldBe Long.MaxValue
          error.getMessage should include("TRAFFIC_UPDATE_OUT_OF_BOUND")
        }
      }
      ...
      "not overflow the total debit value" in {
        val store = mk()
        for {
          accountFetch <- store.persist(alice, debitBalanceDelta(Long.MaxValue), clock.now)
          // Going 1 above Long.MaxValue should fail
          error <- store.persist(alice, debitBalanceDelta(Long.MaxValue), clock.now).failed
        } yield {
      ...
      "not allow negative total debit value" in {
        ...
        for {
          ...
          error <- store.persist(alice, debitBalanceDelta(-1L), clock.now).failed
          events <- store.getEvents(alice, CantonTimestamp.MinValue)
        } yield {
          events shouldBe empty
          error.getMessage should include("TRAFFIC_UPDATE_OUT_OF_BOUND")
      ...
      "leave the balance untouched when an update is rejected" in {
        ...
        val timestamp1 = clock.now
        for {
          _ <- store.persist(alice, creditBalanceDelta(Long.MaxValue), timestamp1)
          _ <- store.persist(alice, creditBalanceDelta(1L), timestamp1.immediateSuccessor).failed
          getBalance <- store.balance(alice)
        } yield
        // The failed update must not have been partially applied
      ...
      "scope by accounts" in {
        ...
        val timestamp2 = timestamp1.immediateSuccessor
        for {
          _ <- store.persist(alice, creditBalanceDelta(10), timestamp1)
          _ <- store.persist(butternut, debitBalanceDelta(5), timestamp2)
      ...
      "keep the most recent timestamp for out of order updates" in {
        ...
        for {
          ...
          afterPersist2 <- store.persist(alice, debitBalanceDelta(3), older)
          getBalance2 <- store.balance(alice)
        } yield {
      ...
      "advance the timestamp when a newer update arrives" in {
        ...
        for {
          _ <- store.persist(alice, creditBalanceDelta(10), timestamp1)
          _ <- store.persist(alice, debitBalanceDelta(3), newer)
          getBalance <- store.balance(alice)
      ...
      "support negative balance deltas" in {
        ...
        val timestamp1 = clock.now
        for {
          _ <- store.persist(alice, debitBalanceDelta(10), timestamp1)
          negativeDebit <- store.persist(alice, debitBalanceDelta(-5), timestamp1)
          getBalanceDebit <- store.balance(alice)
          _ <- store.persist(butternut, creditBalanceDelta(10), timestamp1)
          negativeCredit <- store.persist(butternut, creditBalanceDelta(-3), timestamp1)
      ...
      "retrieve events" in {
        ...
        for {
          ...
          _ <- store.persist(alice, debitBalanceDelta(5), timestamp2)
      ...
      "deduplicate the same (event_source, event_id) across accounts" in {
        ...
        for {
          first <- store.persist(alice, creditBalanceDelta(10), timestamp1, eventId = sharedId)
          // Same (source, id) globally: this is a no-op, butternut is never created
          dedup <- store.persist(butternut, creditBalanceDelta(20), timestamp1, eventId = sharedId)
          _ <- assertAliceHasInitialCredit(store, timestamp1)
          getBalanceButternut <- store.balance(butternut)
