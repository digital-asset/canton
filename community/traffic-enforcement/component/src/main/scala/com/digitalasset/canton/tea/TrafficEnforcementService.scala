          ...
          AccountId
            ...
            .leftMap[RpcError](err =>
              RequestValidationErrors.InvalidArgument
                .Reject(s"invalid account id '${request.accountId}': ${err.message}")
            )
        )
      balance <- EitherT
        .liftF[FutureUnlessShutdown, RpcError, Option[AccountState]](
    ...
    def processBalanceDelta(balanceDelta: Long) = for {
      ...
      eventId <- EitherT.fromEither[FutureUnlessShutdown](
        EventId
          ...
          .leftMap[RpcError](err =>
            RequestValidationErrors.InvalidArgument
              .Reject(s"invalid deduplication id '${request.deduplicationId}': ${err.message}")
          )
      )
      newBalance <- store
        .persistTrafficDelta(
