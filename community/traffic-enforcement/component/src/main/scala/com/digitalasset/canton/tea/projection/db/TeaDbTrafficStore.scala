...
class TeaDbTrafficStore(
  ...
  override def persistTrafficDelta(
    ...
    val reject = TeaDbTrafficStore.rejection(accountId, trafficDelta)
    EitherT(
      storage
        .queryAndUpdate(
          persistDeltaDBIO(
        ...
        .transform {
          case Failure(ex) if reject.isDefinedAt(ex) => Success(Outcome(Left(reject(ex))))
          case Failure(ex) => Failure(ex)
          case Success(outcome) => Success(outcome.map(Right(_)))
        }
    )
  }
