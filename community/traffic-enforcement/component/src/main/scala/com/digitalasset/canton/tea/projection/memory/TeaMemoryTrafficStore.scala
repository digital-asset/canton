...
class TeaMemoryTrafficStore(override val loggerFactory: NamedLoggerFactory)(implicit
  ...
  override def getBalance(accountId: AccountId)(implicit
    ...
    EitherT.rightT(balances.get(BalanceKey(accountId)))
  ...
  override def persistTrafficDelta(
    ...
    EitherT(
      persistDeltaInternal(
        ...
        .recover { case ex =>
          // The in-memory map can't have transient failures, so anything that's not a business rejection is fatal.
          Left(
            TeaMemoryTrafficStore
              .rejection(accountId, trafficDelta)
              .applyOrElse(ex, (t: Throwable) => TrafficEnforcementErrors.FatalFailure.Reject(t))
          )
        }
