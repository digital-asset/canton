  ...
  override def saveCounterCheckpoint(
    ...
    EitherT.cond[Future](
      existingDataO.forall {
        case CheckpointDataAtCounter(existingTs, existingLatestTopologyTs) if existingTs <= ts =>
          (existingLatestTopologyTs, latestTopologyClientTimestamp) match {
            case (Some(existing), Some(updated)) if existing <= updated =>
              true
            case (None, _) =>
              true
            case _ =>
