...
import org.apache.pekko.stream.scaladsl.{Flow, GraphDSL, Keep, Merge, Sink, Source, WireTap}
...
class SendEventGenerator(
  ...
  def generate(
    ...
    val trafficReceiptO = submissionOrOutcome.map(_.trafficReceiptO).getOrElse(None)
...
object PeriodicCheckpointsForAllMembers {
  ...
  /** An Pekko flow that passes the `Traced[BatchWritten]` untouched from input to output,
  ...
  def apply(
    ...
    val logger = TracedLogger(PeriodicCheckpointsForAllMembers.getClass, loggerFactory)
    ...
    Flow.fromGraph(GraphDSL.createGraph(recordCheckpointSink) {
      implicit b: GraphDSL.Builder[(KillSwitch, Future[Done])] => recordCheckpointShape =>
        import GraphDSL.Implicits.*
        val bcast = b.add(WireTap[Traced[BatchWritten]]())
