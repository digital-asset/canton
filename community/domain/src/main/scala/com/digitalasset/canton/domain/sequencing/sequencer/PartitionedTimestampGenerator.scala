...
/** To generate unique timestamps between many nodes without coordination we partition available timestamps by the
  ...
  */
class PartitionedTimestampGenerator(clock: Clock, nodeIndex: Int, nodeCount: Int) {
  ...
  def generateNext: CantonTimestamp = {
    ...
    val micros = lastRef.updateAndGet { latest =>
      val latestMicros = Math.max(nowMicros, latest + nodeCount)
      val uniqueMicros = latestMicros - (latestMicros % nodeCount) + nodeIndex
      uniqueMicros
    }
