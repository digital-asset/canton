...
trait SequencerStore extends SequencerMemberValidator with NamedLogging with AutoCloseable {
  ...
  def readEvents(
    ...
    // DeliverStoreEvent in the fan-out buffer always contains the traffic receipt,
    // which should not be populated for recipients other than the sender.
    def filterTrafficReceipt(event: Sequenced[Payload]): Sequenced[Payload] =
      event.event match {
        case e: DeliverStoreEvent[Payload] =>
          val newEvent = event.copy(event =
            e.copy(trafficReceiptO = e.trafficReceiptO.filter(_ => memberId == e.sender))
          )
          newEvent
        case _ => event
      }
      ...
      case Some(fromExclusive) =>
        cache.headOption match {
          ...
          case _ =>
            ...
            readEventsInternal(member, memberId, fromExclusiveO, limit)
        }
      case None =>
        logger.debug(
