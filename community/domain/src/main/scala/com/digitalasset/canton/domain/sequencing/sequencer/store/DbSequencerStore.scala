...
class DbSequencerStore(
    ...
    // TODO(#16089) TODO(#15837): Check that buffer size is 0 if we are in the HA mode
  ...
  private def readPayloadsFromStore(payloadIds: Seq[PayloadId])(implicit
    ...
    NonEmpty.from(payloadIds) match {
      ...
      case Some(payloadIdsNE) =>
        val queries = DbStorage.toInClauses_("id", payloadIdsNE, maxInClauseSize).map { inClause =>
          val query = sql"""select id, content
          from sequencer_payloads
          where """ ++ inClause
          query.as[Payload]
        }
        storage.sequentialQueryAndCombine(queries, functionFullName).map { payloads =>
  ...
  private def readEventsLatest(
    ...
    def queryEvents(safeWatermarkO: Option[CantonTimestamp]) = {
      ...
      val safeWatermark = safeWatermarkO.getOrElse(CantonTimestamp.MaxValue)
      val query = profile match {
        case _: Postgres | _: H2 =>
          h2PostgresQueryEvents(safeWatermark)
  ...
  /** This method returns `Left` if old "updatable" checkpoints are found, `Right` otherwise.
    */
  private def previousCheckpoints(
      beforeInclusive: CantonTimestamp
  )(implicit traceContext: TraceContext): DBIOAction[
    Either[Unit, (CantonTimestamp, Map[Member, CounterCheckpoint])],
    NoStream,
    Effect.Read,
  ] = {
    ...
    query.as[(Member, CounterCheckpoint)].map { previousCheckpoints =>
      ...
      if (timestamps.size > 1) {
        ...
        // If no such checkpoints are found, we return Left to indicate to use old "updatable" checkpoints.
        logger.debug(
          s"Checkpoint for all members are not aligned, will use slower checkpoint computation. Found timestamps: $timestamps"
        )
        Left(())
      } else {
        Right(
  ...
  private def memberCheckpointsQuery(
    ...
    for {
      previousCheckpointsE <- previousCheckpoints(beforeInclusive)
      (previousCheckpoints, countedEventsSinceCheckpoint) <- {
        previousCheckpointsE match {
          case Right((previousCheckpointTimestamp, previousCheckpoints)) =>
            query(previousCheckpointTimestamp)
              .as[(Member, Long)]
              .map(_.toMap)
              .map((previousCheckpoints, _))
          case Left(_) =>
            // Produce the checkpoints using the old query that takes checkpoints into account internally
            // We provide empty eventCntSince to have final output compute the correct checkpoints (adding +0)
            DbSequencerStoreLegacy
              .oldMemberCheckpointsQuery(storage, beforeInclusive, safeWatermark)
              .map {
                (_, Map.empty[Member, Long])
              }
        }
      }
    } yield {
  ...
  private def memberLatestSequencerTimestampQuery(
    ...
    for {
      previousCheckpointsE <- previousCheckpoints(beforeInclusive)
      (previousCheckpoints, latestSequencerTimestampsSincePreviousCheckpoint) <- {
        previousCheckpointsE match {
          case Right((previousCheckpointTimestamp, previousCheckpoints)) =>
            query(previousCheckpointTimestamp)
              ...
              .map((previousCheckpoints, _))
          case Left(_) =>
            // Produce the latest sequencer client timestamps using the old query that uses checkpoints internally
            // We provide empty previousCheckpoints to have final output compute the correct checkpoints
            DbSequencerStoreLegacy
              .oldMemberLatestSequencerTimestampQuery(
                storage,
                beforeInclusive,
                safeWatermark,
                sequencerId,
              ...
              .map {
                (Map.empty[Member, CounterCheckpoint], _)
              }
        }
      }
    } yield {
