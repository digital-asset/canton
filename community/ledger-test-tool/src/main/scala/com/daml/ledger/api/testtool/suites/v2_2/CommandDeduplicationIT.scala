...
final class CommandDeduplicationIT(
  ...
  testGivenAllParticipants(
    ...
    _ => { case Participants(Participant(ledger, Seq(party))) =>
      ...
      val firstAcceptedSubmissionId = newSubmissionId()
      val beforeFirstCompletionAccepted = System.nanoTime()
      runWithTimeModel { minMaxSkewSum =>
        for {
          ...
          afterSecondCompletionAccepted = System.nanoTime()
          _ <- assertPartyHasActiveContracts(
            ledger,
            party,
            noOfActiveContracts = 2,
          ...
          assert(
            afterSecondCompletionAccepted - beforeFirstCompletionAccepted >= deduplicationDuration.toNanos,
            s"The requested deduplication duration $deduplicationDuration was greater than the time local time difference between the first and second accepted command (${(afterSecondCompletionAccepted - beforeFirstCompletionAccepted) / 1000000}ms).",
          )
          assert(
