...
class VettingIT(testDars: TestDars) extends LedgerTestSuite with AppendedClues {
  import testDars.{
    VettingAltDar,
    VettingDepDar,
  ...
  private def vetAllInDar(
    ...
    participant.updateVettedPackages(
      vetPkgIdsRequest(
        dar.archive.all.map(_._1.toString),
        alternativeSynchronizer = alternativeSynchronizer,
        expectedTopologySerial = expectedTopologySerial,
      )
    )
  ...
  private def opDARMains(
    ...
    val mainPackageIds = dars.map(_.archive.main._1)
  ...
  private def uploadDarFileDontVetRequest(dar: TestDar) =
    UploadDarFileRequest(darFile = dar.bytes, "", DontVetAnyPackages.toProto, "")
  ...
  test(
    ...
    for {
      ...
      _ <- participant.uploadDarFileAndVetOnConnectedSynchronizers(VettingDepDar.bytes)
      _ <- participant.uploadDarFileAndVetOnConnectedSynchronizers(VettingMainDar_1_0_0.bytes)
      _ <- participant.uploadDarFileAndVetOnConnectedSynchronizers(VettingMainDar_2_0_0.bytes)
      _ <- participant.uploadDarFile(uploadDarFileDontVetRequest(VettingAltDar))
  ...
  def updateTest(
    ...
    test(
      ...
      for {
        ...
        _ <- participant.uploadDarFile(uploadDarFileDontVetRequest(VettingMainDar_1_0_0))
  ...
  updateTest(
    ...
    implicit ec =>
      (participant: ParticipantTestContext) =>
        for {
          vetDep <- vetAllInDar(participant, VettingDepDar)
          vetMain <- vetDARMains(participant, Seq(VettingMainDar_2_0_0))
  ...
  updateTest(
    ...
    implicit ec =>
      (participant: ParticipantTestContext) =>
        for {
          _ <- vetAllInDar(participant, VettingMainDar_2_0_0)
          unvetMain <- unvetDARMains(participant, Seq(VettingMainDar_2_0_0))
  ...
  updateTest(
    ...
    implicit ec =>
      (participant: ParticipantTestContext) =>
        for {
          _ <- vetAllInDar(participant, VettingDepDar)
  ...
  updateTest(
    ...
    implicit ec =>
      (ledger: ParticipantTestContext) =>
        for {
          response1 <- vetAllInDar(ledger, VettingDepDar)
  ...
  test(
    ...
    for {
      ...
      _ <- participant
        .validateDarFile(
          ValidateDarFileRequest(
            VettingMainDar_Split_Lineage_2_0_0.bytes,
            "",
      ...
      _ <- participant
        .validateDarFile(
          ValidateDarFileRequest(
            VettingMainDar_3_0_0_Incompatible.bytes,
            "",
            synchronizerId = synchronizerIdOrFail,
          )
        )
  ...
  test(
    ...
    for {
      _ <- setNodeIds(participant)
      _ <- participant.uploadDarFile(uploadDarFileDontVetRequest(VettingMainDar_2_0_0))
      _ <- participant.uploadDarFile(uploadDarFileDontVetRequest(VettingMainDar_3_0_0_Incompatible))
      _ <- participant.uploadDarFile(
        uploadDarFileDontVetRequest(VettingMainDar_Split_Lineage_2_0_0)
      )
  ...
  test(
    ...
    case Participants(Participant(participant1, _), Participant(participant2, _)) =>
      for {
        ...
        _ <- vetAllInDar(participant1, VettingMainDar_2_0_0, Some(sync1))
        _ <- vetAllInDar(participant2, VettingMainDar_2_0_0, Some(sync1))
        _ <- vetAllInDar(participant1, VettingMainDar_2_0_0, Some(sync2))
        _ <- vetAllInDar(participant2, VettingAltDar, Some(sync2))
        ...
        _ <- unvetDARMains(participant1, Seq(VettingMainDar_2_0_0), Some(sync2))
  ...
  test(
    ...
    case Participants(Participant(participant1, _), Participant(participant2, _)) =>
      for {
        ...
        _ <- vetAllInDar(participant2, VettingMainDar_2_0_0, Some(sync2))
        _ <- vetAllInDar(participant1, VettingMainDar_2_0_0, Some(sync2))
        _ <- vetAllInDar(participant2, VettingMainDar_2_0_0, Some(sync1))
        _ <- vetAllInDar(participant1, VettingMainDar_2_0_0, Some(sync1))
  ...
  test(
    ...
    for {
      ...
      vetMain <- vetAllInDar(participant, VettingMainDar_1_0_0)

      _ <- participant.create(party, new MainTSimple_1_0_0(party))

      unvetMain <- unvetDARMains(participant, Seq(VettingMainDar_1_0_0))
