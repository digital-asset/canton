...
object Assertions extends OptionValues with LoneElement {
  ...
  def assertGreaterOrEquals[T](context: String, a: T, b: T)(implicit ordering: Ordering[T]): Unit =
    assert(
      ordering.gteq(a, b),
      s"$context: $a should be equal or greater than $b",
    )
  ...
  /** Match the given exception against a status code and a regex for the expected message. Succeeds
    * if the exception is a GrpcException with the expected code and the regex matches some part of
    * the message or there is no message and the pattern is None.
    */
  @tailrec
  def assertGrpcErrorRegex(
    ...
    t match {
      ...
      case exception: StatusRuntimeException =>
        optPattern.foreach(assertMatches("Error message", exception.getMessage, _))
  ...
  def assertMatches(subjectName: String, message: String, pattern: Pattern): Unit =
    if (pattern.matcher(message).find()) {
      ...
      fail(s"$subjectName did not contain [$pattern], but was [$message].")
    }
