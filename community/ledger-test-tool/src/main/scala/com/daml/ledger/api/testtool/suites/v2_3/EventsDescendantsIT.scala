...
import com.daml.ledger.api.testtool.suites.v2_3.EventsDescendantsIT.isDescendant
...
class EventsDescendantsIT extends LedgerTestSuite {
  ...
  test(
    ...
    for {
      ...
      val lastCreatedEvent = assertSingleton(
        ...
        createdEvents.maxByOption(_.nodeId).toList,
      )
  ...
  test(
    ...
    for {
      ...
      forAll(exercisedEvents)(exercisedEvent =>
        assert(
          isDescendant(
  ...
  test(
    ...
    for {
      ...
      forAll(Seq(first -> second, second -> first)) { case (event1, event2) =>
        assert(
          !isDescendant(
            who = event1.nodeId,
  ...
  test(
    ...
    for {
      ...
      txTreeException <-
        alpha
          .exercise(party, t.exerciseRollbackConsumingWithCreate(tExercise))
          .mustFail("didn't try to rollback exercise")
    } yield {
  ...
  test(
    ...
    for {
      ...
      txTreeException <-
        alpha
          .exercise(party, t.exerciseRollbackCreateWithExercise())
          .mustFail("didn't try to rollback create")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
        ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
