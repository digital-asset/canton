...
import org.scalatest.matchers.should.Matchers.{fail as _, *}
...
final class ExceptionsIT extends LedgerTestSuite {
  ...
  test(
    ...
    for {
      ...
      txTreeException <- ledger
        .exercise(party, t.exerciseRollbackConsuming(tExercise))
        .mustFail("successfully rolled back exercise")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
        ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
      }
    }
  })
  ...
  test(
    ...
    for {
      ...
      txTreeException <- ledger
        .exercise(party, t.exerciseRolledbackArchiveConsuming(withKey))
        .mustFail("successfully rolled back archive")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
        ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
  ...
  test(
    ...
    for {
      ...
      withKey <- ledger.create(party, new WithSimpleKey(party))
      txTreeException <- ledger
        .exercise(party, t.exerciseRolledbackArchiveNonConsuming(withKey))
        .mustFail("successfully rolled back archive")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
  ...
  test(
    ...
    for {
      t <- ledger.create(party, new ExceptionTester(party))
      txTreeException <- ledger
        .exercise(party, t.exerciseRolledbackDuplicateKey())
        .mustFail("successfully rolled back archive")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
  ...
  test(
    ...
    for {
      t <- ledger.create(party, new ExceptionTester(party))
      txTreeException <- ledger
        .exercise(party, t.exerciseRollbackCreate())
        .mustFail("successfully rolled back create")
    } yield {
      inside(txTreeException) { case ex: StatusRuntimeException =>
        ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
        ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
    ...
    case p @ Participants(
      ...
      for {
        ...
        txTreeException <- aLedger
          ...
          .mustFail("Tried to rollback projection normalization")
      } yield {
        inside(txTreeException) { case ex: StatusRuntimeException =>
          ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
          ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
    ...
    case p @ Participants(
      ...
      for {
        ...
        txTreeException <- aLedger
          ...
          .mustFail("Tried to rollback projection nesting")
      } yield {
        inside(txTreeException) { case ex: StatusRuntimeException =>
          ex.getStatus.getCode.toString should include("INVALID_ARGUMENT")
          ex.getStatus.getDescription should startWith("DAML_EFFECTFUL_ROLLBACK_ERROR")
