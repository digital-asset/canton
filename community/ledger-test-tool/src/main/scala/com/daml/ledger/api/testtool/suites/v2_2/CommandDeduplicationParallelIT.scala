...
class CommandDeduplicationParallelIT extends LedgerTestSuite {
  ...
  private def runTestWithSubmission[T](
    ...
    for {
      responses <- Future
        .traverse(Seq.fill(numberOfParallelRequests)(()))(_ => submitRequestAndGetStatus())
      ...
      val duplicateResponses = alreadyExistsResponses + abortedResponses
      assert(
      ...
      assert(activeContracts.sizeIs == 1)
    }
  ...
  private def submitAndWaitRequestAndGetStatusCode(
    ...
    submissionResultToFinalStatusCode(ledger)(submitResult.map(_ => ()), submissionId, parties*)
  }
  ...
  protected def submitRequestAndGetStatusCode(
    ...
    submissionResultToFinalStatusCode(ledger)(submitResult, submissionId, parties*)
  }
  ...
  private def submissionResultToFinalStatusCode(
    ...
    .transformWith {
      ...
      case Success(_) =>
        ledger
          .findCompletion(parties*)(completion => completion.submissionId == submissionId)
          .map {
            case Some(response) =>
              Status.fromCodeValue(response.getStatus.code).getCode
            case None =>
