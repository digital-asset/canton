...
class InterfaceSubscriptionsIT(testDars: TestDars)
    extends InterfaceSubscriptionsITBase(testDars, "IS")
class InterfaceSubscriptionsWithEventBlobsIT(testDars: TestDars)
    extends InterfaceSubscriptionsITBase(testDars, "ISWP")

abstract class InterfaceSubscriptionsITBase(testDars: TestDars, prefix: String)
  ...
  test(
    ...
    for {
      _ <- ledger.uploadDarFileAndVetOnConnectedSynchronizers(testDars.Carbonv1TestDar.bytes)
      ...
      _ <- ledger.uploadDarFileAndVetOnConnectedSynchronizers(testDars.Carbonv2TestDar.bytes)
