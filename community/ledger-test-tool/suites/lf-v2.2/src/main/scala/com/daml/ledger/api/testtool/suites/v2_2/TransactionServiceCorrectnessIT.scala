...
import com.daml.ledger.api.testtool.suites.v2_2.TransactionServiceCorrectnessIT.*
  ...
  test(
    ...
    case p @ Participants(Participant(alpha, Seq(alice)), Participant(beta, Seq(bob))) =>
      for {
        ...
        assert(
          alphaView.head.paidTrafficCost.exists(_ > 0L),
          s"Submitting node for a submitting party should have a positive paid traffic cost, got ${alphaView.head.paidTrafficCost}",
        )
        assert(
          alphaBobView.head.paidTrafficCost.isEmpty,
          s"Submitting node for a non submitting party should have an unset paid traffic cost, got ${alphaBobView.head.paidTrafficCost}",
        )
        assert(
          betaView.head.paidTrafficCost.isEmpty,
          s"Non submitting node should have an unset paid traffic cost, got ${betaView.head.paidTrafficCost}",
        )
      }
  })
