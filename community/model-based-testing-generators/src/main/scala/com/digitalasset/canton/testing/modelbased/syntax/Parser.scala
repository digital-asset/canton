...
package com.digitalasset.canton.testing.modelbased.syntax

import com.digitalasset.canton.testing.modelbased.ast.Concrete.*
import com.digitalasset.canton.testing.modelbased.syntax.Lexer.Token
...
object Lexer extends RegexParsers {
  ...
  final case object QUERY_BY_KEY extends Token
  final case object CIDS extends Token
  final case object EXHAUSTIVE extends Token
  ...
  final case object BRACKET_OPEN extends Token
  final case object BRACKET_CLOSE extends Token
  ...
  private def bracketOpen: Parser[Token] = "[" ^^^ BRACKET_OPEN
  private def bracketClose: Parser[Token] = "]" ^^^ BRACKET_CLOSE
  ...
  private def queryByKey: Parser[Token] = "QueryByKey" ^^^ QUERY_BY_KEY
  private def cids: Parser[Token] = "cids" ^^^ CIDS
  private def exhaustive: Parser[Token] = "exhaustive" ^^^ EXHAUSTIVE
  private def trueToken: Parser[Token] = "true" ^^^ TRUE
  private def falseToken: Parser[Token] = "false" ^^^ FALSE
  ...
  private def processIndentation(tokens: List[Token], indents: List[Int] = List(0)): List[Token] = {
    val indentsHead = indents.headOption.getOrElse(
      throw new IllegalArgumentException("indentation stack cannot be empty")
    )
    tokens match {
      case INDENTATION(n) :: tail =>
        if (n > indentsHead) {
          INDENT :: processIndentation(tail, n :: indents)
        } else if (n < indentsHead) {
  ...
  def tokens: Parser[List[Token]] = phrase(
    rep1(
      indentation | nat | braceOpen | braceClose | bracketOpen | bracketClose
        ...
        | exhaustive | trueToken | falseToken
    )
  ) ^^ { tokens => processIndentation(tokens) }
...
private class TokenReader(tokens: List[Token]) extends Reader[Token] {
  override def first: Token =
    tokens.headOption.getOrElse(throw new IllegalStateException("no more tokens"))
...
object Parser extends Parsers {
  ...
  def contractIdList: Parser[ContractIdList] = for {
    _ <- accept(BRACKET_OPEN)
    cids <- repsep(contractId, COMMA)
    _ <- accept(BRACKET_CLOSE)
  } yield cids
  ...
  def queryByKey(keys: Keys): Parser[(QueryByKey, Option[PackageId])] = for {
    _ <- accept(QUERY_BY_KEY)
    pkgId <- maybePkgKeyVal.commit
    cids <- contractIdList.commit
    rest <- cids.headOption match {
      case Some(firstCid) =>
        for {
          _ <- accept(EXHAUSTIVE).commit
          _ <- accept(EQUAL).commit
          exhaustive <- boolVal.commit
          (keyId, maintainers) = keys(firstCid)
          mismatched = cids.filter(cid => keys(cid) != (keyId, maintainers))
          _ <-
            if (mismatched.isEmpty) success(())
            else
              err(
                s"not all cids in QueryByKey have the same key: cid $firstCid has key ($keyId, $maintainers)" +
                  s" but ${mismatched.map(cid => s"cid $cid has key ${keys(cid)}").mkString(", ")}"
              )
        } yield {
          (keyId, maintainers, exhaustive)
        }
      case None =>
        for {
          _ <- accept(KEY).commit
    ...
    val (keyId, maintainers, exhaustive) = rest
    (QueryByKey(cids, keyId, maintainers, exhaustive), pkgId)
  }
  ...
  def commands(keys: Keys): Parser[(Commands, Keys)] = for {
    ...
    _ <- accept(EQUAL).commit
    disclosures <- contractIdList.commit
  ...
  @nowarn("msg=match may not be exhaustive")
  def parseScenario(s: String): Either[String, Scenario] =
    scenario(new TokenReader(lex(s))) match {
      case Success(res, _) => Right(res)
      case NoSuccess(msg, next) => Left(s"parsing error: $msg at $next")
    }

  @nowarn("msg=match may not be exhaustive")
  def assertParseScenario(s: String): Scenario =
    scenario(new TokenReader(lex(s))) match {
      case Success(res, _) => res
      case NoSuccess(msg, next) =>
