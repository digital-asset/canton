...
object Pretty {
  ...
  object PrettyLedgers {
    ...
    private def commandsToTree(commands: Commands): Tree =
      Tree(
        s"Commands participant=${commands.participantId} actAs=${prettyParties(commands.actAs)} disclosures=${prettyContractIds(commands.disclosures)}",
    ...
    private def actionToTree(action: Action, pkgId: Option[PackageId]): Tree = action match {
      ...
      case QueryByKey(contractIds, keyId, maintainers, exhaustive) =>
        if (contractIds.nonEmpty)
          Tree(
            s"QueryByKey${prettyPackageId(pkgId)} ${prettyContractIds(contractIds)} exhaustive=$exhaustive",
            Nil,
          )
        else
          Tree(
            s"QueryByKey${prettyPackageId(pkgId)} ${prettyContractIds(contractIds)} key=($keyId, ${prettyParties(maintainers)}) exhaustive=$exhaustive",
            Nil,
          )
    ...
    private def prettyContractIds(contractIdList: ContractIdList): String =
      contractIdList.mkString("[", ",", "]")
    ...
    private def actionToTree(action: Action, pkgId: Option[PackageId]): Tree = action match {
      ...
      case QueryByKey(contractIds, keyId, maintainers, exhaustive) =>
        if (contractIds.elements.nonEmpty)
          Tree(
            s"QueryByKey${prettyPackageId(pkgId)} $contractIds exhaustive=$exhaustive",
            Nil,
          )
        else
          Tree(
            s"QueryByKey${prettyPackageId(pkgId)} $contractIds key=($keyId, $maintainers) exhaustive=$exhaustive",
            Nil,
          )
      case Rollback(subTransaction) =>
        Tree(s"Rollback", subTransaction.map(actionToTree(_, None)))
  ...
  def prettyScenario(scenarion: Concrete.Scenario): String =
    PrettyLedgers.scenarioToTree(scenarion).pretty(0)

  def prettyLedger(ledger: Concrete.Ledger): String =
    PrettyLedgers.ledgerToTree(ledger).pretty(0)

  def prettyScenarioSkeleton(scenario: Skeleton.Scenario): String =
    PrettySkeletons.scenarioToTree(scenario).pretty(0)

  def prettySkeletonLedger(ledger: Skeleton.Ledger): String =
