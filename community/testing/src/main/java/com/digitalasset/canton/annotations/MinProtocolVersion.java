...
/**
 * Annotation for declaring that a test suite runs only with the given protocol version or a later
 * one. All tests of an annotated suite are reported as ignored when {@code
...
@Retention(RUNTIME)
@Target(TYPE)
public @interface MinProtocolVersion {
  String value();
}
