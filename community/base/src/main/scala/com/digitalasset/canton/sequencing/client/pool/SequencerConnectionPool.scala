...
/** Pool of sequencer connections.
  ...
  *     validated (see [[InternalSequencerConnection.SequencerConnectionState.Validated]]) and agree
  *     on bootstrap information (synchronizer ID, static parameters) before the pool is initialized
  *     and starts serving connections.
...
trait SequencerConnectionPool extends FlagCloseable with NamedLogging {
  ...
  /** Return the current configuration of the pool.
    */
  def config: SequencerConnectionPoolConfig
  ...
  /** Dynamically update the pool configuration. Changing the expected synchronizer ID is not
    ...
    */
  def updateConfig(newConfig: SequencerConnectionPoolConfig)(implicit
      traceContext: TraceContext
  ): Either[SequencerConnectionPoolError, Unit]

  def health: SequencerConnectionPoolHealth
  ...
  @VisibleForTesting
  def contents: Map[SequencerId, Set[SequencerConnection]]
}

object SequencerConnectionPool {
  ...
  /** Sequencer pool configuration
    ...
    */
  final case class SequencerConnectionPoolConfig(
      connections: NonEmpty[Seq[ConnectionConfig]],
    ...
    import SequencerConnectionPoolConfig.*

    override protected def pretty: Pretty[SequencerConnectionPoolConfig] = prettyOfClass(
    ...
    def validate: Either[SequencerConnectionPoolError.InvalidConfigurationError, Unit] = {
      ...
      check.leftMap(SequencerConnectionPoolError.InvalidConfigurationError.apply)
    }
  ...
  object SequencerConnectionPoolConfig {
    private[sequencing] final case class ChangedConnections(
        added: Set[ConnectionConfig],
        removed: Set[ConnectionConfig],
    ) extends PrettyPrinting {
    ...
    def fromSequencerConnections(
      ...
      new SequencerConnectionPoolConfig(
        connectionsConfig,
  ...
  class SequencerConnectionPoolHealth(
  ...
  object SequencerConnectionPoolHealth {
    class AlwaysHealthy(
        override val name: String,
        protected override val logger: TracedLogger,
    ) extends SequencerConnectionPoolHealth(
          name,
  ...
  sealed trait SequencerConnectionPoolError

  object SequencerConnectionPoolError {
    final case class InvalidConfigurationError(error: String) extends SequencerConnectionPoolError
    final case class TimeoutError(error: String) extends SequencerConnectionPoolError
    final case class ThresholdUnreachableError(error: String) extends SequencerConnectionPoolError
  }
}

trait SequencerConnectionPoolFactory {
