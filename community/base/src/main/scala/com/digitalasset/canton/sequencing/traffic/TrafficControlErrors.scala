...
object TrafficControlErrors extends TrafficControlErrorGroup {
  ...
  @Explanation(
    """The sequencer's traffic control state has not been initialized yet during an logical synchronizer upgrade."""
  )
  @Resolution(
    """Retry until the sequencer has been initialized with traffic."""
  )
  object LsuTrafficNotInitialized
      extends ErrorCode(
    ...
    final case class Error()(implicit
        val loggingContext: ErrorLoggingContext
          ...
          cause =
            "Sequencer is awaiting to be initialized with traffic control state during a logical synchronizer upgrade"
        )
        with TrafficControlError
  }
}
