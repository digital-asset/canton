...
object TopologyStateProcessor {
  ...
  def forTopologyManager[PureCrypto <: CryptoPureApi](
    ...
    if (topologyConfig.useNewProcessor) {
      ...
      new TopologyStateProcessorOld(
        store,
        outboxQueue,
        topologyMappingChecks = oldMappingChecks,
        pureCrypto = pureCrypto,
        loggerFactoryParent = loggerFactoryParent,
      )
    }
  ...
  def forTransactionProcessing[PureCrypto <: CryptoPureApi](
      ...
      topologyConfig: TopologyConfig,
      topologyMappingChecksFactory: TopologyStateLookup => TopologyMappingChecks,
      topologyMappingChecksFactoryOld: => TopologyMappingChecksOld,
      pureCrypto: PureCrypto,
      loggerFactoryParent: NamedLoggerFactory,
  )(implicit ec: ExecutionContext) = if (topologyConfig.useNewProcessor) {
    new TopologyStateProcessorImpl(
      store,
      cache.getOrElse(
        throw new IllegalArgumentException("Cache must exist if we use new processor")
      ),
      outboxQueue = None,
      topologyMappingChecksFactory,
      pureCrypto,
    ...
    new TopologyStateProcessorOld(
      store,
      outboxQueue = None,
      topologyMappingChecksFactoryOld,
      pureCrypto,
      loggerFactoryParent,
    )
  }
