...
sealed abstract case class LightTransactionViewTree private[data] (
  ...
  // Depending on the protocol version, the reference can either be a view hash or a ciphertext ID (PV`transparency`+).
...
object LightTransactionViewTree
  ...
  def toFullViewTrees[A, B, C](
      ...
      // [[FullTransactionViewTree]]. For other protocol versions lower than PV`transparency`,
      // where no ciphertext ID is available, we must use the view hash instead.
          ...
          .traverse { case (viewHash, key) =>
            byCiphertextIdMapO match {
              case Some(byCiphertextIdMap) =>
                // For PV`transparency`+, we use the ciphertext ID as the reference for subviews. The ciphertext ID is a hash of the
                // ciphertext (generated by encrypting all views with the same recipient tree) combined with its
                // relative "position" before encryption. We no longer use the view hash as a reference in PV`transparency`+,
  ...
  /** Builds a LightTransactionViewTree using Ciphertext ID-based references (PV`transparency`+
    * mode).
    *
