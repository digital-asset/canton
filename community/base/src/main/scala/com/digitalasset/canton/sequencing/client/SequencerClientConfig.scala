...
/** Client configured options for how to connect to a sequencer
  *
  * @param eventInboxSize
  *   The size of the inbox queue used to store received events. Must be at least one. Events in the
  *   inbox are processed in parallel. A larger inbox may result in higher throughput at the price
  *   of higher memory consumption, larger database queries, and longer crash recovery.
  * @param startupConnectionRetryDelay
  ...
  *   Controls how frequently the client acknowledges how far it has successfully processed to the
  *   sequencer which allows the sequencer to remove this data when pruning.
  ...
  * @param skipSequencedEventValidation
  *   if true, sequenced event validation will be skipped. the default setting is false. this option
  *   should only be enabled if a defective validation is blocking processing. therefore, unless you
  ...
  *   will process simultaneously, preventing overload and ensuring that the system can handle the
  *   workload effectively. A higher value of `maximumInFlightEventBatches` can lead to increased
