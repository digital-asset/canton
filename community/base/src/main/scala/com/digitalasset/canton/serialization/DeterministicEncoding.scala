...
sealed trait DeserializationError extends PrettyPrinting {
  ...
  def toProtoDeserializationError: ProtoDeserializationError =
    this match {
      ...
      case CryptoParseAndValidationError(message) =>
        ProtoDeserializationError.CryptoParseAndValidationError(message)
...
final case class CryptoParseAndValidationError(message: String) extends DeserializationError
