...
/** A Canton protocol version is a snapshot of how the Canton protocols, that nodes use to
  * communicate, function at a certain point in time (e.g., this ‘snapshot’ contains the information
  * what exactly a `SubmissionRequest` to the sequencer looks like and how exactly a Sequencer
  * handles a call of the `SendAsync` RPC). It is supposed to capture everything that is involved in
  * two different Canton nodes interacting with each other.
  *
  * The protocol version is important for ensuring we meet our compatibility guarantees such that we
  * can
  ...
  *
  *   - The new protocol version should be declared as alpha until it is released: Define it with
  *     type argument [[com.digitalasset.canton.version.ProtocolVersionAnnotation.Alpha]] and add it
  *     to the list in [[com.digitalasset.canton.version.ProtocolVersion.alpha]].
  *
  *   - Add a new test job for the protocol version `N` to the canton_build workflow. Make a
  *     sensible decision how often it should run. If sensible, consider to reduce the frequency
  *     some of the other protocol version test jobs are running, e.g., by moving them to the
  *     canton_nightly job.
  *
  * How to release a protocol version `N`:
  *   - Switch the type parameter of the protocol version constant `v<N>` from
  *     [[com.digitalasset.canton.version.ProtocolVersionAnnotation.Alpha]] to
  *     [[com.digitalasset.canton.version.ProtocolVersionAnnotation.Stable]] As a result, you may
  *     have to modify a couple of protobuf definitions and mark them as stable as well.
  *
  *   - Remove `v<N>` from [[com.digitalasset.canton.version.ProtocolVersion.alpha]] and add it to
  *     [[com.digitalasset.canton.buildinfo.BuildInfo.stableProtocolVersions]].
  *
  * How to release a protocol version `N` as Beta:
  *   - Switch the type parameter of the protocol version constant `v<N>` from
  *     [[com.digitalasset.canton.version.ProtocolVersionAnnotation.Alpha]] to
  *     [[com.digitalasset.canton.version.ProtocolVersionAnnotation.Beta]]
  *   - Remove `v<N>` from [[com.digitalasset.canton.version.ProtocolVersion.alpha]] and add it to
...
object ProtocolVersion {
  ...
  /** Parse a given raw version string into a [[ProtocolVersion]] without any further validation,
    * i.e. it allows to create invalid and unsupported [[ProtocolVersion]]!
    *
    * ONLY use this method when
    *   - implementing functionality for the [[ProtocolVersion]] itself
    *   - additional validation is being applied on the resulting [[ProtocolVersion]] afterwards as
    *     a exception
  ...
  /** Creates a [[ProtocolVersion]] from the given raw version value and ensures that it is a
    ...
    *   String to be parsed.
    * @param allowDeleted
    *   If true, don't fail if `rawVersion` corresponds to a deleted protocol version. This should
    *   only be used when parsing a version that does not correspond to the one running on the
    *   synchronizer. One such example is the minimum supported protocol version from a participant.
    * @return
    */
  def create(
      rawVersion: String,
  ...
  /** Like [[create]] ensures a supported protocol version; but throws a runtime exception for
    * errors.
    */
