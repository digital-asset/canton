...
import com.digitalasset.canton.sequencing.client.pool.{
  ...
  SequencerSubscriptionWrapperFactory,
...
/** A sequencer subscription wrapper implementation that replays events previously stored and sends
  * them to the handler.
  *
  ...
  */
private[sequencing] class ReplaySequencerSubscriptionWrapper private[sequencing] (
    override val connection: SequencerConnection,
    replayPath: Path,
    handler: MaybeCompressedSequencedEventHandler[SequencerClientSubscriptionError],
    protected override val timeouts: ProcessingTimeout,
    protected override val loggerFactory: NamedLoggerFactory,
)(implicit ec: ExecutionContext)
    extends SequencerSubscriptionWrapper {
  import ReplaySequencerSubscriptionWrapper.ReplayStatistics
  ...
  override def start()(implicit traceContext: TraceContext): Either[String, Unit] = {
    ...
    val replayF = MonadUtil
      ...
      .map { _ =>
        ...
        ReplaySequencerSubscriptionWrapper.replayStatistics.add(
...
object ReplaySequencerSubscriptionWrapper {
  @VisibleForTesting
...
/** A sequencer subscription wrapper implementation that does nothing.
  *
  * @param connection
  *   the underlying connection to the sequencer
  */
private[sequencing] class NoOpSequencerSubscriptionWrapper private[sequencing] (
    override val connection: SequencerConnection,
    protected override val timeouts: ProcessingTimeout,
    protected override val loggerFactory: NamedLoggerFactory,
) extends SequencerSubscriptionWrapper {
  ...
  override private[sequencing] val health: HealthComponent =
    new AlwaysHealthyComponent(s"no-op-subscription-${connection.name}", logger)
...
private[sequencing] class ReplaySequencerSubscriptionWrapperFactory(
  ...
  override def create(
    ...
    new ReplaySequencerSubscriptionWrapper(
...
private[sequencing] class NoOpSequencerSubscriptionWrapperFactory(
    timeouts: ProcessingTimeout,
    loggerFactory: NamedLoggerFactory,
) extends SequencerSubscriptionWrapperFactory {
  override def create(
    ...
    new NoOpSequencerSubscriptionWrapper(
      connection = connection,
      timeouts = timeouts,
