...
class SequencerConnectionPoolImpl private[sequencing] (
    private val initialConfig: SequencerConnectionPoolConfig,
    connectionFactory: InternalSequencerConnectionFactory,
    clock: Clock,
  ...
  import SequencerConnectionPoolImpl.*
  ...
  private val configRef = new AtomicReference[SequencerConnectionPoolConfig](initialConfig)
  ...
  private val pool = mutable.Map[SequencerId, NonEmpty[Seq[SequencerConnection]]]()

  /** Tracks the configured connections, along with their validated status */
  private val trackedConnections: mutable.Map[InternalSequencerConnection, Boolean] =
    mutable.HashMap[InternalSequencerConnection, Boolean]()
  ...
  private val initializedP = {
    ...
    PromiseUnlessShutdown
      .supervised[Either[SequencerConnectionPoolError, Unit]](
        "connection-pool-initialization",
        futureSupervisor,
      )
  }

  override def config: SequencerConnectionPoolConfig = configRef.get

  override val health: SequencerConnectionPoolHealth = new SequencerConnectionPoolHealth(
  ...
  override def isThresholdStillReachable(
      ...
      traceContext: TraceContext
  ): Either[SequencerConnectionPoolError.ThresholdUnreachableError, Unit] =
    lock.exclusive {
      ...
      Either.cond(
        isReachable,
        (),
        SequencerConnectionPoolError.ThresholdUnreachableError(message),
      )
    }
  ...
  private class ConnectionHandler(connection: InternalSequencerConnection) extends NamedLogging {
    protected override val loggerFactory: NamedLoggerFactory =
      SequencerConnectionPoolImpl.this.loggerFactory
      ...
      connection.health
        ...
        .registerHighPriorityOnHealthChange(new HealthListener {
          ...
          override def poke()(implicit traceContext: TraceContext): Unit = {
            ...
            state match {
              case SequencerConnectionState.Validated => processValidatedConnection(connection)

              case SequencerConnectionState.Stopped(_) =>
              ...
              case SequencerConnectionState.Fatal(_) | SequencerConnectionState.Initial |
                  SequencerConnectionState.Starting | SequencerConnectionState.Started |
                  SequencerConnectionState.Stopping(_) =>
                if (state.isFatal)
  ...
  override def updateConfig(
      newConfig: SequencerConnectionPoolConfig
  )(implicit
      traceContext: TraceContext
  ): Either[SequencerConnectionPoolError.InvalidConfigurationError, Unit] =
    lock.exclusive {
      ...
      for {
        ...
        _ <- Either.cond(
          ...
          (),
          SequencerConnectionPoolError.InvalidConfigurationError(
        ...
        bootstrapIfThresholdReachedO <-
          if (poolNotInitialized && newThreshold < currentConfig.trustThreshold) {
            getBootstrapIfThresholdReached(
              ...
              .leftMap(SequencerConnectionPoolError.InvalidConfigurationError.apply)
          } else {
            Right(None)
          }
      } yield {
  ...
  private def markValidated(
      connection: InternalSequencerConnection
  )(implicit traceContext: TraceContext): Unit = {
    {
      lock.exclusive {
  ...
  private def getBootstrapIfThresholdReached(
      threshold: PositiveInt,
      ignored: Set[ConnectionConfig] = Set.empty,
  ...
  private def processValidatedConnection(connection: InternalSequencerConnection)(implicit
      traceContext: TraceContext
  ): Unit =
    lock.exclusive {
...
class GrpcSequencerConnectionPoolFactory(
