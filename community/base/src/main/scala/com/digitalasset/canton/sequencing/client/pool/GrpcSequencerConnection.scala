...
import com.digitalasset.canton.sequencing.client.pool.Connection.{ConnectionConfig, ConnectionError}
...
class GrpcSequencerConnection(
    private val underlying: GrpcInternalSequencerConnection,
    override val name: String,
    stub: UserSequencerConnectionStub,
  ...
  override val health: SequencerConnectionHealth = underlying.health

  override val config: ConnectionConfig = underlying.config
  ...
  private def maybeFailConnection(
    ...
    error match {
      ...
      case ConnectionError.TransportError(grpcError: GrpcServiceUnavailable) =>
  ...
  override def acknowledgeSigned(
    ...
    stub
      ...
      .recover {
        ...
        case SequencerConnectionStubError.ConnectionError(ConnectionError.TransportError(x))
