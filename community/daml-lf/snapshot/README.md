...
temporary file with a name similar to `$SNAPSHOT_DIR/ReplayBenchmark.dar/globalCreateAndExercise/snapshot-participant1__12207e2b741ddb807d24dae8a7480bca51e61aa094b299517aa3f3d56eded6fc9ea7.bin`.

Note: here we assume that a `sbt "daml-lf-snapshot/Test/compile"` has already been ran to generate the `ReplayBenchmark.dar` Dar file.

A choice may now be benchmarked by JMH using:

    sbt "daml-lf-snapshot/Jmh/run                                                                         \
      -p entriesFile=$SNAPSHOT_DIR/ReplayBenchmark.dar/globalCreateAndExercise/snapshot-participant1*.bin \
