...
This benchmarks the LF engine using transactions from a collection of ledger entries stored in a file.

## Build

Build the tool with sbt:

    sbt "daml-lf-snapshot/Test/compile"
...
* `<exercise choice names>`: is the qualified choice name of the
  root exercise node to be benchmarked separated by commas (`,`). A
  ...
  By default, the tool benchmarks the first choice with such a name it finds in the ledger export.
...
* the optional parameter `<directory with dar file(s)>` specifies the full path of
  a directory containing all the dar files to be used. If defined
...
A sample ledger entries file can be generated by running:
     ...
     sbt "daml-lf-snapshot/testOnly *GenerateSnapshots"

This will save snapshotting information (i.e. the ledger entries file) to a
temporary file with a name similar to `$SNAPSHOT_DIR/snapshot-participant1__12207e2b741ddb807d24dae8a7480bca51e61aa094b299517aa3f3d56eded6fc9ea7.bin`.

Note: here we assume that a `sbt "daml-lf-snapshot/Test/compile"` has already been ran to generate the `ReplayBenchmark.dar` Dar file in the `$SAVED` directory.

A choice may now be benchmarked by JMH using:

    sbt "daml-lf-snapshot/Jmh/run                                                 \
      -p entriesFile=$SNAPSHOT_DIR/snapshot-participant1*.bin \
...
## Extracting and Filtering Choice Names

Top level choice names can be extracted from a snapshot file using:

     STANDALONE=True                                      \
     SNAPSHOT_DIR=/tmp/canton/snapshot                    \
     sbt "daml-lf-snapshot/testOnly *ExtractSnapshotChoices"

Limited choice name filtering is possible by setting the following additional environment variables:

* `MIN_STEP_COUNT` - specifies the minimum number of engine execution steps (in total) taken when executing a top level choice

* `MIN_TX_NODE_COUNT` - specifies the minimum number of transaction nodes (in total) that a top level choice interacts with.

# `replay-profile`

This profiles the replay of a transaction built from a ledger entries file.

     sbt "daml-lf-snapshot/runMain com.digitalasset.daml.lf.testing.snapshot.ReplayProfile \
