...
/** SExpr0 -- AST for the speedy compiler pipeline.
  ...
  *
  * These are the expression forms which exist during the speedy compiler pipeline: The pipeline
  * stages are:
  *
  * 1: convert from LF
  ...
  *
  * 2: closure conversion 3: transform to ANF
  *
  * Stage 1 is in PhaseOne.scala Stage 2 is in ClosureConversion.scala Stage 3 is in Anf.scala
  *
  * During Stage2 (Closure Conversion), we move from SExpr0 to SExpr1, During Stage3 (ANF
  * transformation), we move from SExpr1 to SExpr.
  *
  * Summary of which constructors are contained by: SExp0, SExpr1 and SExpr:
  *
  *   - In SExpr{0,1,} (everywhere): SEApp(General), SEBuiltin, SELabelClosure, SELocation,
  *     SEScopeExercise, SETryCatchV1, SEVal, SEValue,
  *
  ...
  *
  *   - In SExpr: SEAppAtomicGeneral, SEAppAtomicSaturatedBuiltin, SECaseAtomic, SELet1Builtin,
  *     SELet1BuiltinArithmetic
  *
...
private[speedy] object SExpr0 {
  ...
  /** Reference to a variable. 'level' is the 0-based de Bruijn LEVEL (not INDEX)
    * https://en.wikipedia.org/wiki/De_Bruijn_index This expression form is only allowed prior to
    * closure conversion
    */
