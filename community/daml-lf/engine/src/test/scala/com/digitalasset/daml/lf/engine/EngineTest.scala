...
import com.digitalasset.daml.lf.engine.Error.{Interpretation as IErr, Validation as VErr}
...
import com.digitalasset.daml.lf.interpretation.{ExecutionMode, InterpretationConfig, Limits}
...
class EngineTestCidV1 extends EngineTest(ContractIdVersion.V1, ExecutionMode.UpdateMachine)
class EngineTestCidV2 extends EngineTest(ContractIdVersion.V2, ExecutionMode.UpdateMachine)
class EngineTestKeyCidV1 extends EngineTestKey(ContractIdVersion.V1, ExecutionMode.UpdateMachine)
class EngineTestKeyCidV2 extends EngineTestKey(ContractIdVersion.V2, ExecutionMode.UpdateMachine)

class EngineTestExceptionsKeyCidV1
    extends EngineTestExceptionsKey(ContractIdVersion.V1, ExecutionMode.UpdateMachine)
class EngineTestExceptionsKeyCidV2
    extends EngineTestExceptionsKey(ContractIdVersion.V2, ExecutionMode.UpdateMachine)
...
class EngineTestExceptionsKeyCmdCidV2
    extends EngineTestExceptionsKey(ContractIdVersion.V2, ExecutionMode.Conductor)
...
abstract class EngineTest(contractIdVersion: ContractIdVersion, executionMode: ExecutionMode)
    extends AnyWordSpec
    with Matchers
  ...
  val helpers =
    new EngineTestHelpers(contractIdVersion, executionMode, "BasicTests-nokey.dar", loggerFactory)
  import helpers.*
  ...
  "minimal create command" should {
    ...
    val res = preprocessor
      ...
      .consume(lookupHandler(lookupContract, lookupPackage, lookupKey))
  ...
  "multi-party create command" should {
    ...
    "not allow replay with a subset of submitters" in {
      forAll(cases) { case (templateId, signatories, submitters) =>
        ...
        inside(replayResult.start) { case Result.Step.Error(_) => succeed }
      }
    }
  }
  ...
  "exercise command" should {
    ...
    "be validated with no change in metrics" in {
      ...
      val freshEngine1 = newEngine(executionMode)
      val freshEngine2 = newEngine(executionMode)
  ...
  "dynamic fetch actors" should {
    ...
    "not mark any node as byKey" in {
      runExample(fetcher2Cid, clara).map { case (tx, _) =>
        byKeyNodes(tx).size
      } shouldBe Right(0)
    }
  }
  ...
  "reinterpreting fetch nodes" should {
    ...
    "succeed with a fresh engine, correctly compiling packages" in {
      val engine = newEngine(executionMode)
  ...
  "wrongly typed contract id" should {
    ...
    def run(cmds: ImmArray[ApiCommand]) =
      suffixLenientEngine
        ...
        .consume(lookupHandler(contracts, lookupPackage, lookupKey))
  ...
  "nested transactions" should {
    ...
    def run(n: Int) = {
      ...
      suffixLenientEngine
        ...
        .consume(lookupHandler(PartialFunction.empty, lookupPackage, PartialFunction.empty))
    }
  ...
  "Engine.preloadPackage" should {
    ...
    val (_, _, allPackagesDev) =
      new EngineTestHelpers(contractIdVersion, executionMode, "BasicTests-keys.dar", loggerFactory)
        .loadAndAddPackage(
    ...
    s"accept stable packages from $devVersion even if version is smaller than min version" in {
      ...
      } yield inside(eng.preloadPackage(pkgId, pkg).start) { case Result.Step.Pure(_) =>
        succeed
      }
    }
    ...
    s"reject stable packages from $devVersion if version is greater than max version" in {
      for {
        ...
        pkgId = pkg.packageId
        pkg <- allPackagesDev.get(pkgId).toList
      } yield inside(eng.preloadPackage(pkgId, pkg).start) {
        case Result.Step.Pure(_) => pkg.languageVersion shouldBe <=(lv)
        case Result.Step.Error(_) => pkg.languageVersion shouldBe >(lv)
      }
    }
  }
  ...
  "transaction tree limits" should {
    val submissionSeed = hash("exercise command")
    val templateId = Identifier(basicTestsPkgId, "BasicTests:SimpleMultiParty")
    val let = Time.Timestamp.now()
    val seeding = Engine.initialSeeding(submissionSeed, participant, let)
    val cid = toContractId("BasicTests:SimpleMultiParty:1")
    val choiceName = "HelloMultiParty"
    val choiceArg = ValueRecord(None, ImmArray.Empty)
    val command =
      ApiCommand.Exercise(
        templateId.toRef,
        cid,
    ...
    val submitters = Set(alice, bob)
    val res = preprocessor
      .preprocessApiCommands(Map.empty, ImmArray(command))
      .consume(lookupHandler(lookupContract, lookupPackage, lookupKey))
    res shouldBe a[Right[?, ?]]
    val limits = Limits.Lenient.copy(choiceAuthorizers = 1)
    val engineWithLimits = newEngine(executionMode, limits = limits)
    val contractInst =
      TransactionBuilder.fatContractInstanceWithDummyDefaults(
        version = defaultSerializationVersion,
        contractId = cid,
        ...
        arg = ValueRecord(
          None,
          ImmArray((None /* p1 */, ValueParty(alice)), (None /* p2 */, ValueParty(bob))),
        ),
        signatories = List(alice, bob),
      )
    def lookUpContractInst = Map(contractInst.contractId -> contractInst)
    ...
    "engine submissions" should {
          ...
          res
            .flatMap { cmds =>
              engineWithLimits
                ...
                .consume(lookupHandler(lookUpContractInst, lookupPackage, lookupKey))
            }

        inside(interpretResult) { case Left(VErr(VErr.TransactionLimitExceeded(errors))) =>
          errors shouldBe Set(
      ...
      // TODO (#23280): ensure all transaction tree validators can fail on submission
      // TODO (#23280): ensure all transaction tree validators can succeed on validation
    }
  }
}
...
abstract class EngineTestKey(
  ...
  "exercise-by-key" should {
    ...
    "crash if use a contract key with an empty set of maintainers" in {
      ...
      val result = suffixStrictEngine
        ...
        .consume(lookupHandler(PartialFunction.empty, lookupPackage, lookupKey))
    ...
    "translate key with unscaled numeric" in {
      translateAndCheck(
        Seq(
          ...
          (
            Some[Ref.Name]("n4"),
            ValueNumeric(com.digitalasset.daml.lf.data.Numeric.assertFromString("10.12")),
          ),
          (Some[Ref.Name]("i"), ValueNone),
        )
      )
  ...
  "global key lookups" should {
    ...
    def run(cmd: ApiCommand): Int = {
      ...
      val Right(cmds) = preprocessor
        .preprocessApiCommands(Map.empty, ImmArray(cmd))
        .consume(lookupHandler(contracts, allPkgs, mockedKeyLookup))
      val result = suffixStrictEngine
        .executeCommands(
          validating = false,
          submitters = submitters,
  ...
  "fetching contracts that have keys correctly fills in the transaction structure" when {
    val fetchedCid = withKeyContractInst.contractId
    val now = Time.Timestamp.now()
    ...
    "fetched via a fetch" in {
      ...
      lazy val lookupContractMap = Map(withKeyContractInst.contractId -> withKeyContractInst)
      ...
      val Right((tx, _, _)) = suffixLenientEngine
        ...
        .consume(lookupHandler(lookupContractMap, lookupPackage, lookupKey))
    ...
    "fetched via a fetchByKey" in {
      ...
      val fetcherInst = TransactionBuilder.fatContractInstanceWithDummyDefaults(
        version = defaultSerializationVersion,
        contractId = toContractId("BasicTests:FetcherByKey 2"),
        packageName = basicTestsPkg.pkgName,
      ...
      val fetcherCid = fetcherInst.contractId

      val lookupContractMap = Map(
...
class EngineTestExceptionsKey(contractIdVersion: ContractIdVersion, executionMode: ExecutionMode)
    extends EngineTestExceptions(contractIdVersion, executionMode, "Exceptions-keys.dar")
...
class EngineTestExceptions(
  ...
  def run(cmd: ApiCommand) = {
    ...
    val Right(cmds) = preprocessor
      .preprocessApiCommands(Map.empty, ImmArray(cmd))
      .consume(lookupHandler(contracts, allExceptionsPkgs, lookupKey))
      ...
      forEvery(negativeTestCases)((v, min, max) =>
        inside(
          engine(LanguageVersion.allLfVersions.filter(lv => lv >= min && lv <= max))
            .preloadPackage(pkgId, pkg(v))
            .start
        ) { case Result.Step.Pure(_) => succeed }
      )
    }
  }
}
...
class EngineTestHelpers(
    contractIdVersion: ContractIdVersion,
    executionMode: ExecutionMode = ExecutionMode.UpdateMachine,
  ...
  val suffixStrictEngine: Engine =
    newEngine(requireCidSuffixes = true, executionMode = executionMode)
  val suffixLenientEngine = newEngine(executionMode = executionMode)
  ...
  def reinterpret(
    ...
    val finalState =
      actionNodes.foldLeft[Either[Error, ReinterpretState]](
        Right(ReinterpretState(contracts, keys))
      ) { case (acc, nodeId) =>
        for {
          state <- acc
    ...
    def commit(tr: Tx, meta: Tx.Metadata): ReinterpretState = {
      val (newContracts, newKeys) = tr.fold((contracts, keys)) {
        ...
        case ((contracts, keys), (_, create: Node.Create)) =>
          val coinst = FatContractInstance
