...
import com.digitalasset.daml.lf.transaction.{GlobalKeyWithMaintainers, NodeId}
...
final case class CreateEvent(
  ...
  /** Note that the stakeholders of each event node will always be a subset of the event witnesses.
    * We perform this narrowing since usually when consuming these events we only care about the
    * parties that were included in the disclosure information. Consumers should be aware that the
    * stakeholders stored are _not_ all the stakeholders of the contract, but just the stakeholders
    * "up to witnesses".
    *
    * For broader and more detailed information, the consumer can use [[signatories]] and/or
    * [[observers]].
    */
...
/** Event for exercises
  ...
  * @param children
  *   consequence events. note that they're paired with the NodeId of the transaction that
  ...
  *   result of exercise of the choice, none for aborted exercise.
  */
