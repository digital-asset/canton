...
/** The Engine configurations describes the versions of language and transaction the engine is
  * allowed to read and write together with engine debugging feature.
  *
  * <p>
  *
  * @param allowedLanguageVersions
  *   The range of language versions the engine is allowed to load. The engine will crash if it
  *   asked to load a language version that is not included in this range.
  * @param transactionTraceMaxLenght
  *   Specified the maximum length of the stack trace reported in case of interpretation error.
  * @param stackTraceMode
  *   The flag enables the runtime support for stack trace.
  * @param profileDir
  *   The optional specifies the directory where to save the output of the Daml scenario profiler.
  *   The profiler is disabled if the option is empty.
  * @param snapshotDir
  *   The optional specifies the directory where Daml snapshots will be saved. Snapshots are
