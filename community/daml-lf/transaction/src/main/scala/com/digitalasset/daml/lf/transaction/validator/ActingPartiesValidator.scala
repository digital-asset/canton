...
final case class ActingPartiesValidator(
  ...
  override def validateNode(node: Node): Option[Limit.Error] =
    node match {
      case node: Node.Fetch if (node.actingParties.size > limits.actingParties) =>
        Some(
          Limit.ActingParties(
            node.coid,
            node.templateId,
      ...
      case node: Node.Exercise if (node.actingParties.size > limits.actingParties) =>
        Some(
          Limit.ActingParties(
            node.targetCoid,
            node.templateId,
