...
/** Implements a state machine for contracts and their keys while interpreting a Daml-LF command or
  ...
  * contract state machine keeps track of the updates to the
  * [[LegacyContractStateMachine.ActiveLedgerState]] since the beginning of the interpretation or
  * iteration. Given a [[LegacyContractStateMachine.State]] `s`, a client can compute the next state
  * for a given action node `n`, by calling `s.handleNode(..., n, ...)`. For a rollback node `nr`, a
  * client must call `beginRollback` before processing the first child of `nr` and `endRollback`
  * after processing the last child of `nr`.
  *
  * @tparam Nid
  ...
  *   [[com.digitalasset.daml.lf.transaction.LegacyContractKeyUniquenessMode.Strict]] and
  * @see
  *   ContractStateMachineSpec.visitSubtree for iteration in all modes
  */
object LegacyContractStateMachine {
  ...
  /** @param locallyCreated
    ...
    * after a rollback scope is left. Formally, if a contract state machine transitions from state
    * `s1` to state `s2`, then `s1.globalKeyInputs` is a subset of `s2.globalKeyInputs`.
    *
    * The map `globalKeyInputs` can be used to resolve keys during re-interpretation.
    *
    * In mode [[com.digitalasset.daml.lf.transaction.LegacyContractKeyUniquenessMode.Strict]],
    ...
    *
    * In mode [[com.digitalasset.daml.lf.transaction.LegacyContractKeyUniquenessMode.Off]],
    ...
    *   - If `n` is lookup-by-key, then `globalKeyInputs(k)` is [[Transaction.KeyActive]] (positive
    *     lookup) or [[Transaction.NegativeKeyLookup]] (negative lookup).
    *   - If `n` is a create, then `globalKeyInputs(k)` is [[Transaction.KeyCreate]]. Note: a plain
    *     fetch or exercise (with `byKey == false`) does not impact `globalKeyInputs`.
    *
    * @param activeState
    *   Summarizes the active state of the partial transaction that was visited so far. When a
    *   rollback scope is left, this is restored to the state at the beginning of the rollback,
    *   which is cached in `rollbackStack`.
    *
    * @param rollbackStack
    *   The stack of active states at the beginning of the currently active try blocks
    *   (interpretation) or Rollback nodes (iteration).
    *
    * Invariants:
    *   - [[globalKeyInputs]]'s keyset is a superset of [[activeState]].[[ActiveLedgerState.keys]]'s
    *     keyset and of all the [[ActiveLedgerState.keys]]'s keysets in [[rollbackStack]]. (the
    *     superset can be strict in case of by-key nodes inside an already completed Rollback scope)
    *   - In mode [[LegacyContractKeyUniquenessMode.Strict]], the keys belonging to the contracts in
    *     [[activeState]].[[ActiveLedgerState.consumedBy]]'s keyset are in
    *     [[activeState]].[[ActiveLedgerState.keys]], and similarly for all [[ActiveLedgerState]]s
    *     in [[rollbackStack]].
    */
  case class State[Nid] private[lf] (
      ...
      activeState: LegacyContractStateMachine.ActiveLedgerState[Nid],
      rollbackStack: List[LegacyContractStateMachine.ActiveLedgerState[Nid]],
      mode: LegacyContractKeyUniquenessMode,
  ) {
    ...
    /** Lookup the given key k. Returns
      ...
      */
    def lookupActiveKey(key: GlobalKey): Option[LegacyContractStateMachine.KeyMapping] =
      activeState.getLocalActiveKey(key).orElse(lookupActiveGlobalKeyInput(key))

    def lookupActiveGlobalKeyInput(key: GlobalKey): Option[LegacyContractStateMachine.KeyMapping] =
      globalKeyInputs.get(key).map {
        case Transaction.KeyActive(cid) if !activeState.consumedBy.contains(cid) =>
          LegacyContractStateMachine.KeyActive(cid)
        case _ => LegacyContractStateMachine.KeyInactive
      }
      ...
      if (locallyCreated.union(inputContractIds).contains(contractId)) {
        ...
        mbKey match {
          ...
          case Some(gk) =>
            ...
            Either.cond(
              !conflict || mode == LegacyContractKeyUniquenessMode.Off,
              me.copy(
    ...
    /** Omits the key lookup that are done in
      * [[com.digitalasset.daml.lf.speedy.Compiler.compileChoiceByKey]] for by-bey nodes, which
    ...
    private[lf] def visitExercise(
      ...
      for {
        state <-
          if (byKey || mode == LegacyContractKeyUniquenessMode.Strict)
    ...
    def handleLookup(lookup: Node.QueryByKey): Either[KeyInputError, State[Nid]] = {
      ...
      visitLookup(lookup.gkey, lookup.result.asCidOption, lookup.result.asCidOption).left
        .map(KeyInputError.inject)
    }

    /** Must be used to handle lookups iff in
      * [[com.digitalasset.daml.lf.transaction.LegacyContractKeyUniquenessMode.Off]] mode The second
      * argument takes the contract key resolution to be given to the Daml interpreter instead of
      * `com.digitalasset.daml.lf.transaction.Node.LookupByKey.result`. This is because the
      * iteration might currently be within a rollback scope that has already archived a contract
      * with the key without a by-key operation (and for this reason the archival is not tracked in
      * `LegacyContractStateMachine.ActiveLedgerState.localKeys`),
      * i.e., the lookup resolves to [[scala.None$]] but the correct key input is [[scala.Some$]]
      * for some contract ID and this may matter after the rollback scope is left. Daml Engine will
      * ask the ledger in that case to resolve the lookup and then perform an activeness check
    ...
    def handleLookupWith(
      ...
      visitLookup(lookup.gkey, keyInput, lookup.result.asCidOption).left.map(KeyInputError.inject)
    }
    ...
    /** Utility method that takes a node and computes the corresponding next state. The method does
      ...
      * @param keyInput
      *   will only be used in mode [[LegacyContractKeyUniquenessMode.Off]] and if the node is a
      *   lookupByKey
      */
    def handleNode(
        id: Nid,
    ...
    /** Let `this` state be the result of iterating over a transaction `tx` until just before a node
      ...
      *
      * @param resolver
      *   In mode [[LegacyContractKeyUniquenessMode.Strict]], this parameter has no effect. In mode
      *   [[LegacyContractKeyUniquenessMode.Off]], `resolver` must be the resolver used while
      *   iterating over `tx` until just before `n`. While iterating over the subtree rooted at `n`,
      *   [[projectKeyResolver]](`resolver`) must be used as resolver.
      *
      * @param substate
              ...
              .find {
                case (key, KeyCreate) =>
                  lookupActiveKey(key).exists(
                    _ != KeyInactive
                  ) && mode == LegacyContractKeyUniquenessMode.Strict
  ...
  object State {
    def empty[Nid](mode: LegacyContractKeyUniquenessMode): State[Nid] = new State(
      Set.empty,
      Set.empty,
      Map.empty,
      LegacyContractStateMachine.ActiveLedgerState.empty,
      List.empty,
      mode,
    )
  }
  ...
  /** Summarizes the updates to the current ledger state by nodes up to now.
    ...
    *   a subset of [[LegacyContractStateMachine.State.locallyCreated]].
    *
    * @param consumedBy
