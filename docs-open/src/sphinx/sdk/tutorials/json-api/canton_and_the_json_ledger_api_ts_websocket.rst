...
.. _tutorial-canton-and-the-json-ledger-api-ts-websocket:

=======================================================================
Get Started with Canton, the JSON Ledger API, TypeScript and WebSockets
=======================================================================

This tutorial shows you how to interact with a Canton Ledger using the JSON Ledger API, WebSockets and TypeScript.
...
You should be familiar with the basics of TypeScript, WebSockets and tools such as `npm` and `node.js`.
...
WebSockets Highlights
=====================

The WebSocket connection establishment logic is mentioned in `src/index_websocket.ts`:
...
In `src/index_websocket.ts`, contracts are queried like this:
    ...
    return new Promise<components["schemas"]["CreatedEvent"][]>((resolve, reject) => {
        ...
        return connectWs("/v2/state/active-contracts").then(ws => {
            ...
            ws.onmessage = function (msg: any) {
                ...
                let contract: components["schemas"]["CreatedEvent"] | undefined = undefined;
                if ("JsActiveContract" in contractEntry){
                    contract = contractEntry.JsActiveContract.createdEvent as components["schemas"]["CreatedEvent"];
                }
...
The, execute the `npm run scenario_websocket` once again.
