...
set_skip_output () {
  local value="$1"
  if [[ "$IS_GHA" == "true" && -n "${GITHUB_OUTPUT:-}" ]]; then
    echo "skip=${value}" >> "$GITHUB_OUTPUT"
  fi
}

if [[ "$IS_GHA" == "true" ]]; then
  REPO="${GITHUB_REPOSITORY}"
  # Extract PR number from GITHUB_REF (e.g., refs/pull/12345/merge)
  PULL_NUMBER="${GITHUB_EVENT_PR_NUMBER:-}"
  if [[ -z "$PULL_NUMBER" && "$GITHUB_REF" == refs/pull/* ]]; then
    PULL_NUMBER=$(echo "$GITHUB_REF" | grep -o -E '[0-9]+' | head -1)
  fi
else
