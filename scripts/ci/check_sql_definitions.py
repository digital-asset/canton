...
pg_exceptions = {
    "V1_2__initial_views.sql": ["returns varchar"],
    "V2_1__lapi_3.0_views.sql": ["returns varchar"],
}
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Check sql definitions for max length and collation"
    )
...
def check_statement_pg(fname, line):
    ...
    if (
        "text" in lline
        and not ignore(fname, line, pg_exceptions)
        and re.match(".*\\s+text\\s+\\.*", lline)
    ):
        raise Exception("text definition prohibited, please use varchar instead")
    if (
        "varchar" in lline
        and not ignore(fname, line, pg_exceptions)
        and re.match(".*\\s+varchar\\(\\d+\\).*", lline)
    ):
        raise Exception(
            "fixed length varchar-s are prohibited, please remove the length definition"
        )
    if (
        "varchar" in lline
        and not ignore(fname, line, pg_exceptions)
        and not re.match(".*\\s+collate\\s+\"c\".*", lline)
    ):
...
def check_statement_h2(fname, line):
    ...
    if (
        "varchar" in lline
        and not ignore(fname, line, h2_exceptions)
        and re.match(".*\\s+varchar\\(\\d+\\).*", lline)
    ):
