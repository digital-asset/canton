...
class TimeoutSpec extends AsyncFlatSpec with Matchers {
  ...
  it should "time out, and result should be taken from onTimeout" ignore {
  ...
  // TODO (#30846): disabled flaky test
  it should "time out, and exception should be taken from onTimeout" ignore {
    val resultPromise = Promise[Int]()
    Future {
      Thread.sleep(100)
      10
  ...
  // TODO (#30846): disabled flaky test
  it should "not time out if timeout is not reached, and onTimeout should not be executed" ignore {
    val resultPromise = Promise[Int]()
