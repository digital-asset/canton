...
object FreePort {
  ...
  /** Find a free port.
    *
    * This determines whether a port is free by attempting to bind it and immediately releasing it
    * again. Note, this does not guarantee that the port is still free after this function returns,
    * see [[LockedFreePort]].
    *
    * Draws ports from outside the dynamic port range (port 0) to avoid collisions with parts of the
    * code base that directly bind to port 0.
    */
  def find(): Port = {
