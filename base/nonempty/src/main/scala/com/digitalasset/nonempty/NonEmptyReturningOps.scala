...
/** Functions where ''the receiver'' is non-empty can be found implicitly with no further imports;
  * they "just work". However, if we wish to refine a method on a built-in type that merely
  * ''returns'' a nonempty-annotated type, we must import the contents of this object.
  */
