...
sealed abstract class NonEmptyColl {
  ...
  /** Use its alias [[com.digitalasset.nonempty.NonEmpty]]. */
  type NonEmpty[+A]

  /** Use its alias [[com.digitalasset.nonempty.NonEmptyF]]. */
...
object NonEmpty {
  ...
  /** Usable proof that [[NonEmpty]] is a subtype of its argument. (We cannot put this in an
    ...
    * stdlib ones.)
    */
  def subtype[A]: As[NonEmpty[A], A] = {
    type K[F[_]] = As[F[A], A]
    NonEmptyColl.Instance.subst[K](As.refl[A])
  }
  ...
  /** Usable proof that [[NonEmptyF]] is actually equivalent to its [[NonEmpty]] parent type, not a
    ...
    */
  def equiv[F[_], A]: Is[NonEmpty[F[A]], NonEmptyF[F, A]] = {
    type K[T[_]] = Is[T[F[A]], F[A]]
    type KR[T[_]] = Is[NonEmpty[F[A]], T[A]]
    import NonEmptyColl.Instance.{subst, substF}
    substF[KR, F](subst[K](Is.refl[F[A]]))
  }
...
/** If you ever have to import [[NonEmptyColl]] or anything from it, your Scala settings are
  * probably wrong.
  */
object NonEmptyColl extends NonEmptyCollInstances with catsinstances.impl.CatsInstances {
  ...
  implicit final class ReshapeOps[F[_], A](private val nfa: NonEmpty[F[A]]) extends AnyVal {
    ...
    def toNEF: NonEmptyF[F, A] = NonEmpty.equiv[F, A].coerce(nfa)
  }
...
sealed abstract class NonEmptyCollInstances0 {
  ...
  /** This adds the rest of the `A` API to `na`. However, because it is in the most distant parent
    ...
    */
  implicit def widen[A](na: NonEmpty[A]): A = NonEmpty.subtype[A].coerce(na)
  implicit def widenF[F[+_], A](na: F[NonEmpty[A]]): F[A] =
    As.co[F, NonEmpty[A], A](NonEmpty.subtype[A]).coerce(na)
}
