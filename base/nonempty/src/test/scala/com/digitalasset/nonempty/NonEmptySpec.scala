...
class NonEmptySpec extends AnyWordSpec with Matchers with WordSpecCheckLaws {
  ...
  "Foldable" should {
    "be preferred if Reducible isn't need" in {
      import cats.implicits.*
      cats.Foldable[NonEmptyF[List, *]].getClass should be theSameInstanceAs cats
        .Foldable[List]
        .getClass
    }
  }
  ...
  object UnambiguousResolutionTests {
    ...
    def foldableAll[F[_]: cats.Reducible: cats.Traverse] = cats.Foldable[NonEmptyF[F, *]]
    def functorTraverse[F[_]: cats.Traverse] = cats.Functor[NonEmptyF[F, *]]
  }
}
...
object NonEmptySpec {
  ...
  implicit def `nonempty f eq`[F[_], A](implicit FA: cats.Eq[F[A]]): cats.Eq[NonEmptyF[F, A]] = {
    type T[k[_]] = cats.Eq[k[A]]
