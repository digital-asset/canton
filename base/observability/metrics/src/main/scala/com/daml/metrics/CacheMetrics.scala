...
final class CacheMetrics(cacheName: String, factory: LabeledMetricsFactory) {
  ...
  def registerSizeGauge(sizeSupplier: () => Long)(implicit metricsContext: MetricsContext): Unit = {
    val _ = registerCloseableSizeGauge(sizeSupplier)(metricsContext)
  }

  def registerCloseableSizeGauge(
      sizeSupplier: () => Long
  )(implicit metricsContext: MetricsContext): Gauge.CloseableGauge =
    factory
      .closeableGaugeWithSupplier(
  ...
  def registerWeightGauge(
    ...
    val _ = registerCloseableWeightGauge(weightSupplier)(metricsContext)
  }

  def registerCloseableWeightGauge(
      weightSupplier: () => Long
  )(implicit metricsContext: MetricsContext): Gauge.CloseableGauge =
    factory
      .closeableGaugeWithSupplier(
