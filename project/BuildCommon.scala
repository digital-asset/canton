...
object BuildCommon {
    ...
    .task {
      ...
      val apiFiles =
        ledgerApiProto ++ communityBaseProto ++ communityParticipantProto ++ communityAdminProto ++ communitySynchronizerProto ++
          ...
          damlLfArchiveProto ++ damlLfTransactionProto ++ versionedApiSpecs ++ trafficEnforcementProto
  ...
  lazy val sharedSettings = Seq(
    ...
    // Make sure that shared dependencies with Daml repo force their version
    dependencyOverrides ++= Dependencies.resolveDependency.sharedDependencyMap.values.toSeq,
    publish / skip := true,
  )
  ...
  object CommunityProjects {
    ...
    lazy val `community-app` = project
      ...
      .settings(
        ...
        Test / damlJavaCodegen := Seq(
          ...
          (
            (Test / sourceDirectory).value / "daml" / "CantonExternalCall",
            (Test / damlDarOutput).value / "CantonExternalCall-1.0.0.dar",
    ...
    lazy val `community-participant` = project
      ...
      """
        ...
        Test / useVersionedDarName := true,
        Test / damlDarOutput := resourceManaged.value,
        Test / damlJavaCodegen := Seq(
          (
            (Test / sourceDirectory).value / "daml" / "ModelConformanceExamples" / "V1",
            (Test / damlDarOutput).value / "ModelConformanceExamples-1.0.0.dar",
            "com.digitalasset.canton.damltests.modelconformance.v1",
          )
        ),
    ...
    lazy val `ledger-json-client` = project
      ...
      .settings(
        ...
        Test / sourceGenerators += Def.task {
          ...
          if (filtered.isEmpty)
            sys.error(
              "OpenAPI code generation produced no model files. " +
                "This usually means openapi.yaml has validation errors (check the [error] output above). " +
                "A common cause is incorrect YAML indentation. " +
                "Regenerate with: sbt \"ledger-json-api/Test/runMain com.digitalasset.canton.http.json.GenerateJSONApiDocs\""
            )
          filtered
        }.taskValue,
      )
      ...
      .settings(
        ...
        Compile / damlJavaCodegen := Seq(
          (
            ...
            "com.daml.ledger.test.java.benchtool",
          )
        ),
      )
    ...
    lazy val `model-based-testing-integration-tests` = project
      ...
      .dependsOn(
        `community-app` % "test->test",
        `model-based-testing-drivers`,
        DamlProjects.`daml-lf-transaction-tests` % "test->test",
      )
      .settings(
        sharedCantonCommunitySettings,
        excludeTranscodeConflictingDependencies,
  ...
  object DamlProjects {
    ...
    lazy val allProjects = Set(
      ...
      `daml-lf-interpreter`,
      `daml-lf-interpreter-bench`,
      `daml-lf-engine`,
      `daml-lf-upgrades-matrix`,
      `daml-lf-parser`,
      `daml-lf-encoder`,
    ...
    lazy val `rs-grpc-bridge` = project
      ...
      .settings(
        ...
        libraryDependencies ++= Seq(
          ...
          reactivestreams,
          slf4j_api,
          reactivestreams_tck % Test,
          reactivestreams_examples % Test,
    ...
    lazy val `daml-lf-data-bench` = project
      .in(file("community/daml-lf/data-bench"))
      .disablePlugins(WartRemover)
      .enablePlugins(JmhPlugin)
      .dependsOn(`daml-lf-data`)
    ...
    lazy val `daml-lf-language` = project
      ...
      .settings(
        ...
        Compile / sourceGenerators += DamlLfFeature.generateFeaturesScala.taskValue,
        Compile / resourceGenerators ++= Seq(
          DamlLfVersion.generateVersionsJson.taskValue,
          DamlLfFeature.generateFeaturesJson.taskValue,
        ),
        sharedCommunitySettings,
        scalacOptions := lf_scalaopts_stricter,
        wartremoverErrors := damlWarts,
    ...
    lazy val `daml-lf-archive` = project
      ...
      .settings(
        ...
        // TODO(#30144): replace with @nowarn once the bazel targets are deleted
        Test / scalacOptions ++= Seq("-Wconf:msg=match may not be exhaustive:s"),
        Test / damlDarLfVersions := Seq("2.1"),
        Test / damlExtractMainDalf := true,
        Test / resourceGenerators += DamlPlugin
          .testingDarsFromS3("DarReaderTest-v115", "d1675f71a8bc0c7822eea2d6626cd064d5acfbbc"),
      )
      .dependsOn(
        `daml-lf-data`,
        `daml-lf-language`,
    ...
    // accommodate the following dependency structure:
    // * `daml-lf-snapshot` depends on `daml-lf-engine`, `daml-lf-snapshot-proto`
    // * `daml-lf-engine` depends on `daml-lf-snapshot-proto`
    ...
    lazy val `daml-lf-snapshot` = project
      ...
      .dependsOn(
        ...
        nameof,
        `scala-utils`,
        // TODO (#30325): consider migrating PartiesAllocator to community-integration-testing
        CommunityProjects.`community-app` % "test->test",
      )
      .settings(
        sharedCommunitySettings,
        enablePublishLibrary,
        ...
        Compile / scalacOptions ++= Seq(
          "-Wconf:msg=match may not be exhaustive:s",
          "-Wconf:msg=unused value of type scopt.OptionDef:s",
        ), // TODO(#30144)
        Test / scalacOptions ++= Seq("-Wconf:msg=match may not be exhaustive:s"), // TODO(#30144)
        addProtobufFilesToHeaderCheck(Compile),
        excludeTranscodeConflictingDependencies,
        libraryDependencies ++= Seq(
          google_protobuf_java,
        ...
        Test / damlJavaCodegen := Seq(
          (
            ...
            (Test / damlDarOutput).value / "ReplayBenchmark.dar",
    ...
    lazy val `daml-lf-interpreter` = project
      ...
      .settings(
        ...
        // CostModelTest uses java.lang and java.math via reflection, so we need
        // to --add-opens to these tests.
        // To avoid setting add-opens on all of our SBT builds and tests, we
        // fork the JVM here and set the flags on only that forked JVM.
      ...
      .dependsOn(
        ...
        `daml-lf-validation`,
        `daml-lf-parser`,
        `daml-lf-transaction` % "test->test;test->compile",
        `contextualized-logging`,
        crypto,
        nameof,
        `scala-utils`,
    ...
    lazy val `daml-lf-interpreter-bench` = project
      .in(file("community/daml-lf/interpreter/bench"))
      .disablePlugins(WartRemover)
      .enablePlugins(JmhPlugin)
      .settings(
        sharedCommunitySettings,
    ...
    lazy val `ledger-api-scala` = project
      ...
      .settings(
        ...
        headerResources / excludeFilter := "*.proto",
      )
    ...
    lazy val `bindings-java` = project
      ...
      .settings(
        ...
        libraryDependencies ++= Seq(
          ...
          slf4j_api,
          checkerFramework,
          protoc_gen_grpc_java asProtocPlugin (),
          javax_annotations,
        ),
        headerResources / excludeFilter := "*.proto",
      )
