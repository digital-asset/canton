...
object DamlPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    // Parallel build settings and tasks
    val parallelDamlBuild =
      taskKey[Boolean]("Task to build all DARs in parallelDamlBuildSourceInput")
    val parallelDamlBuildSourceInput =
      settingKey[File]("Directory to get source for a parallel build")
    val parallelDamlBuildDarsOutput =
      settingKey[File]("Directory to put DARs generated by a parallel build")
  }
  ...
  private lazy val baseDamlPluginSettings: Seq[Def.Setting[_]] = Seq(
    ...
    (Test / parallelDamlBuild) := parallelDamlBuildDarsTask.value,
    (Test / parallelDamlBuildSourceInput) := (Test / sourceDirectory).value / "damlParallel",
    (Test / parallelDamlBuildDarsOutput) := target.value / "damlParallelDars",
  )

  // For now, this task is entirely separate from the rest of the DamlPlugin and
  // is only used to build multi-package projects in parallel, by driving `dpm` directly.
  // In the long-term, it would be best if this build process worked alongside
  // all of the other tasks in DamlPlugin.
  def parallelDamlBuildDarsTask = Def.task {
    damlInstall.value
    val damlVersion = damlCompilerVersion.value
    val targetRoot = (Test / parallelDamlBuildDarsOutput).value

    import CacheImplicits._
    val cacheStore = streams.value.cacheStoreFactory.make("damlParallel")
    implicit val inputSingletonCache: sbt.util.SingletonCache[(Set[HashFileInfo], String, String)] =
      sbt.util.SingletonCache.basicSingletonCache(
        tuple3Format(
          immSetFormat(HashFileInfo.format),
          StringJsonFormat, // damlVersion
          StringJsonFormat, // targetRoot
        )
      )

    val upgradesTestFiles = (Test / parallelDamlBuildSourceInput).value
    val damlFiles = (upgradesTestFiles ** "*.daml").get
    val yamlFiles = (upgradesTestFiles ** "daml.yaml").get
    val multiPackageFile = (upgradesTestFiles / "multi-package.yaml")
    val cacheInputFiles = Seq(
      damlFiles,
      yamlFiles,
      Seq(multiPackageFile.getAbsoluteFile),
    ).flatten.toSet.map(FileInfo.hash.apply)
    val cacheInput = (cacheInputFiles, damlVersion, targetRoot.getAbsolutePath)

    val check = Tracked.inputChanged(cacheStore) {
      (changed: Boolean, _: (Set[HashFileInfo], String, String)) =>
        val shouldRebuild = changed || !targetRoot.exists
        if (shouldRebuild) {
          if (targetRoot.exists) IO.delete(targetRoot)
          buildAllDarsInParallel(upgradesTestFiles, targetRoot, damlVersion)
        }
        shouldRebuild
    }
  ...
  // A single entry from the `dpm damlc generate-multi-package-manifest` output
  private case class DamlManifestEntry(
      packageName: String,
      packageVersion: String,
      packageSrc: String,
      output: String,
      darDeps: Seq[String],
      packageDeps: Seq[String],
  )

  private object DamlManifestEntry {
    implicit val entryDecoder: Decoder[DamlManifestEntry] = deriveDecoder[DamlManifestEntry]
  }

  /** Build all DAR packages found under `workingDir` (a multi-package project) in parallel,
    * respecting their dependency order, then copy the resulting DARs into `parallelDarsOutput`.
    */
  private def buildAllDarsInParallel(
      workingDir: File,
      parallelDarsOutput: File,
      damlVersion: String,
  ): Unit = {
    val tempTargetRoot = Files.createTempDirectory("damlParallelDars").toFile
    // Every `dpm` invocation below targets the same temporary root, so that the final
    // result can be copied atomically into `parallelDarsOutput` once everything succeeds.
    val buildEnv = Seq(
      "TARGET_ROOT" -> tempTargetRoot.getAbsolutePath,
      "DAML_VERSION" -> damlVersion,
    )

    // Build a single package
    def build(pkg: DamlManifestEntry): String = {
      runCommand(
        Seq("dpm", "build", "--enable-multi-package", "no"),
        new File(workingDir, pkg.packageSrc),
        buildEnv,
      )(s"dpm build failed for ${pkg.output}")
      pkg.output
    }

    // Extract the build manifest
    val rawManifest = runCommand(
      Seq("dpm", "damlc", "generate-multi-package-manifest"),
      workingDir,
      buildEnv,
    )("dpm damlc generate-multi-package-manifest failed")

    val entries = parser
      .decode[Seq[DamlManifestEntry]](rawManifest)
      .fold(
        e =>
          throw new MessageOnlyException(
            "Invalid JSON from damlc generate-multi-package-manifest: $e"
          ),
        identity,
      )

    // Map package names (<name>-<version>) to where their DARs go in the file system
    val pkgNameToOutput: Map[String, String] =
      entries.map(pkg => s"${pkg.packageName}-${pkg.packageVersion}" -> pkg.output).toMap

    val outputToPkg: Map[String, DamlManifestEntry] = entries.map(pkg => pkg.output -> pkg).toMap

    // Generate the package dependency graph: output -> set of outputs it depends on
    val depGraph: Map[String, Set[String]] = entries.map { pkg =>
      val prereqs = pkg.darDeps.toSet ++ pkg.packageDeps.map(pkgNameToOutput)
      pkg.output -> prereqs
    }.toMap

    // Build packages in parallel, only starting a package's build once all of its
    // dependency outputs (per depGraph) have finished building, mirroring a
    // reverse-topological-order consumption of the dependency graph.
    val executor = Executors.newFixedThreadPool(java.lang.Runtime.getRuntime.availableProcessors())
    try {
      implicit val ec: ExecutionContext = ExecutionContext.fromExecutor(executor)
      val futuresByOutput = mutable.Map[String, Future[String]]()

      def futureFor(output: String): Future[String] =
        futuresByOutput.getOrElseUpdate(
          output, {
            val prereqs = depGraph.getOrElse(output, Set())
            Future
              .sequence(prereqs.toSeq.map(futureFor))
              .flatMap(_ => Future(build(outputToPkg(output))))
          },
        )

      Await.result(Future.sequence(entries.map(pkg => futureFor(pkg.output))), Duration.Inf)
    } finally executor.shutdown()

    // Copy the built DARs back to the expected output location
    IO.copyDirectory(tempTargetRoot, parallelDarsOutput)
  }
