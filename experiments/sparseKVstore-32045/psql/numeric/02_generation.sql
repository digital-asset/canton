...
BEGIN
    ...
    SELECT 
        ...
        key_id,
        -- NUMERIC PACKING: 
        -- We shift the TS 20 decimal places to the left to make room for the tail
        (ts::numeric * 100000000000000000000) + tie::numeric as ts,
        CASE WHEN random() > 0.1 THEN gen_random_bytes(16) ELSE NULL END,
        CASE WHEN random() > 0.1 THEN gen_random_bytes(16) ELSE NULL END,
        -- REPLACES_TS PACKING:
        CASE WHEN key_update_seq > 1 THEN
            (prev_ts::numeric * 100000000000000000000) + prev_tie::numeric
        ELSE NULL END
    FROM linked_updates;
